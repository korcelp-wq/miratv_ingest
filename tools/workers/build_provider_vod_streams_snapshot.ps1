...
.DESCRIPTION
    Provider inventory snapshot worker for VOD streams. Larger than category snapshots, but still read-only/no DB import.
    ...
    Flow:
      ...
      3. Call provider player_api.php?action=get_vod_streams.
      4. Save raw snapshot under runtime/provider_snapshots/vod_streams.
    ...
    This worker:
      - DOES call provider player_api.php for get_vod_streams only.
      - DOES NOT import to database.
      - DOES NOT call get_vod_streams.
    ...
    Intended clean-repo location:
      tools\workers\build_provider_vod_streams_snapshot.ps1
#>

[CmdletBinding()]
param(
    [string]$WorkerName = "build_provider_vod_streams_snapshot",
    [string]$Component = "provider_vod_streams_snapshot",
    ...
    [string]$OutputRoot = "runtime/reports/provider_vod_streams_snapshot",
...
function New-RunIdLocal {
    param([string]$Prefix = "provider-vod-streams-snapshot")
...
function Invoke-JsonGetLocal {
    param(
        [string]$Url,
        [int]$TimeoutSec = 45,
        [string]$UserAgent = "MiraTV-MasterControl-VodStreamSnapshot/1.0"
    )
...
function Invoke-RawGetLocal {
    ...
    $headers = @{
        "Accept" = "application/json"
        "User-Agent" = "MiraTV-MasterControl-VodStreamSnapshot/1.0"
    }
...
function Get-NormalizedVodStreamTextLocal {
    param([string]$Text)
        ...
        $normalizedRows = @(
            $items |
                ForEach-Object {
                    ...
                    elseif ($_.PSObject.Properties.Name -contains "vod_id") {
                        $streamId = [string]$_.vod_id
                    }
                    ...
                    if ($_.PSObject.Properties.Name -contains "container_extension") {
                        $containerExtension = [string]$_.container_extension
                    }

                    if ($_.PSObject.Properties.Name -contains "added") {
                        $added = [string]$_.added
                    }

                    $streamId = $streamId.Trim()
                    $name = $name.Trim()
                    $categoryId = $categoryId.Trim()
                    $containerExtension = $containerExtension.Trim()
                    $added = $added.Trim()

                    if (-not [string]::IsNullOrWhiteSpace($streamId) -or -not [string]::IsNullOrWhiteSpace($name)) {
                        "$streamId|$categoryId|$containerExtension|$added|$name"
                    }
                } |
                Sort-Object -Unique
        )
...
function Get-NormalizedVodStreamHashLocal {
    param([string]$Text)

    $normalized = Get-NormalizedVodStreamTextLocal -Text $Text
...
$startedAt = Get-Date
$signalName = "provider_vod_streams_snapshot_completed"
...
try {
    ...
    if ($loggingAvailable) {
        ...
        if (-not $killEnabled) {
            Write-JobLog `
                ...
                -Status "blocked" `
                -Data @{
                    event_message = "Provider VOD streams snapshot blocked by kill switch."
                    kill_switch_name = $KillSwitchName
                    mac_user_id = $MacUserId
                    provider_label = $provider
                } | Out-Null

            Write-Output "BLOCKED: provider VOD streams snapshot blocked. run_id=$script:RunId kill_switch=$KillSwitchName"
            exit 0
        }
        ...
        Write-JobLog `
            ...
            -Status "started" `
            -Data @{
                event_message = "Provider VOD streams snapshot started."
                mac_user_id = $MacUserId
                provider_label = $provider
    ...
    if (-not [bool]$credentials.username_present -or -not [bool]$credentials.password_present) {
        throw "Resolved account context did not include provider credentials required for VOD stream pull. backend_credential_resolution_used=$backendCredentialResolutionUsed backend_credential_resolution_ok=$backendCredentialResolutionOk backend_error=$backendCredentialResolutionError"
    }
    ...
    if (-not $providerResult.ok) {
        throw "Provider get_vod_streams failed. url=$redactedProviderUrl status_code=$($providerResult.status_code) error=$($providerResult.error)"
    }

    if ([string]::IsNullOrWhiteSpace([string]$providerResult.body)) {
        throw "Provider get_vod_streams returned an empty body. url=$redactedProviderUrl status_code=$($providerResult.status_code)"
    }
    ...
    $snapshotPath = Join-Path $accountSnapshotDir "vod_streams_$stamp.json"
    $summaryJson = Join-Path $outputRootFull "provider_vod_streams_snapshot_summary_$stamp.json"
    $reportCsv = Join-Path $outputRootFull "provider_vod_streams_snapshot_report_$stamp.csv"
    ...
    $normalizedSha256 = Get-NormalizedVodStreamHashLocal -Text ([string]$providerResult.body)
    ...
    if ($null -ne $prior) {
        ...
        $priorNormalizedHash = Get-NormalizedVodStreamHashLocal -Text ([string]$priorBody)
    ...
    if ($loggingAvailable) {
        ...
        Write-JobLog `
            ...
            -Status $statusValue `
            -Data @{
                event_message = "Provider VOD streams snapshot completed."
                provider_calls = $true
                db_imported = $false
                mac_user_id = $MacUserId
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/build_provider_vod_streams_snapshot.ps1" `
            -Data @{
                dashboard_panel = "Provider Sync"
                widget_key = "provider.vod.streams.snapshot"
                owner = "Content Ops"
                kill_switch_name = $KillSwitchName
                provider_calls = $true
                db_imported = $false
    ...
    Write-Output ("OK: provider VOD streams snapshot completed. status=pass provider_calls=True db_imported=False mac_user_id={0} provider_label=""{1}"" action=get_vod_streams item_count_estimate={2} change_status={3} changed={4} backend_credential_resolution_used={5} output_root=""{6}"" run_id={7}" -f `
        $MacUserId, `
        $resolvedProvider, `
        $itemCount, `
        $changeStatus, `
    ...
    if ($loggingAvailable) {
        Write-JobLog `
            ...
            -Status "failed" `
            -Data @{
                event_message = "Provider VOD streams snapshot failed."
                error = $errorMessage
                mac_user_id = $MacUserId
