<#
.CONTRACT-SIGNALS
  provider_snapshot_live_streams_import_preview_completed
  live_streams_delta_import_preview_disposition
  live_streams_delta_import_preview_planned_import_count
  live_streams_delta_import_preview_provider_noise_count
.SYNOPSIS
  Preview Live stream delta import from item-level provider snapshot rows.

.DESCRIPTION
  Governed dry-run preview worker for Live streams.

  This worker fixes the prior routing problem:
    - source_dryrun_csv is a lane/control artifact and is kept only for lineage.
    - source_snapshot is the item-level Live JSON snapshot and is used as the row source.

  It emits one preview CSV row per item-level Live stream record, bounded by -Limit.
...
$WorkerName = "import_live_streams_delta_preview"
...
$PlannedImportCountSignal = "live_streams_delta_import_preview_planned_import_count"
$ProviderNoiseCountSignal = "live_streams_delta_import_preview_provider_noise_count"

$StartedAt = Get-Date
$RunId = "$WorkerName-$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssfffZ'))-$([guid]::NewGuid().ToString('N').Substring(0, 16))"
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$OutputRoot = Join-Path $RepoRoot "runtime\reports\live_streams_delta_import_preview"
$LogRoot = Join-Path $RepoRoot "runtime\logs\live_streams_delta_import_preview"
...
function Get-JsonItems {
    ...
    foreach ($propertyName in @("items", "data", "streams", "live_streams", "result", "rows")) {
...
function Resolve-LatestLiveSnapshot {
    $snapshotRoot = Join-Path $RepoRoot "runtime\provider_snapshots\live_streams"
...
function Resolve-LineageFromLatestDryRun {
    ...
    if (($sourceDryrunCsv -and (Test-Path -LiteralPath $sourceDryrunCsv)) -and [string]::IsNullOrWhiteSpace($sourceSnapshot)) {
        try {
            ...
            $liveRow = $dryRows |
                Where-Object {
                    ((Get-Text -Object $_ -Name "lane_key" -Default "") -eq "live_streams") -or
                    ((Get-Text -Object $_ -Name "media_type" -Default "") -eq "live")
                } |
                Select-Object -First 1

            if ($liveRow) {
                $sourceSnapshot = Get-Text -Object $liveRow -Name "source_snapshot" -Default ""
                if ([string]::IsNullOrWhiteSpace($sourceSnapshot)) {
                    $sourceSnapshot = Get-Text -Object $liveRow -Name "latest_snapshot" -Default ""
                }
                if ([string]::IsNullOrWhiteSpace($sourceSnapshot)) {
                    $sourceSnapshot = Get-Text -Object $liveRow -Name "snapshot_path" -Default ""
                }
            }
        }
        catch {}
    ...
    if ([string]::IsNullOrWhiteSpace($sourceSnapshot) -or -not (Test-Path -LiteralPath $sourceSnapshot)) {
        $sourceSnapshot = Resolve-LatestLiveSnapshot
    }
...
function Get-ProviderLabelFromSnapshotPath {
    ...
    for ($i = 0; $i -lt $parts.Count; $i++) {
        if ($parts[$i] -eq "live_streams" -and ($i + 2) -lt $parts.Count) {
...
function Convert-LiveItemToPreviewRow {
    param(
        [object]$Item,
        [int]$RowNumber,
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($sourceSnapshot) -or -not (Test-Path -LiteralPath $sourceSnapshot)) {
        throw "Unable to resolve readable source_snapshot for Live streams."
    }
    ...
    foreach ($item in ($items | Select-Object -First $Limit)) {
        $rowNumber++
        $previewRows += Convert-LiveItemToPreviewRow `
            -Item $item `
            -RowNumber $rowNumber `
    ...
    $status = "pass"
    $disposition = "live_streams_delta_item_preview_completed"

    if ($plannedImportCount -eq 0) {
        $status = "warning"
        $disposition = "live_streams_delta_item_preview_no_planned_imports"
    }

    $timestamp = (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
    $outputCsv = Join-Path $OutputRoot "live_streams_delta_import_preview_$timestamp.csv"
    $outputJson = Join-Path $OutputRoot "live_streams_delta_import_preview_$timestamp.json"
    $summaryJson = Join-Path $OutputRoot "live_streams_delta_import_preview_summary_$timestamp.json"
    ...
    if (-not $Quiet) {
        Write-Output "OK: Live streams delta item preview completed. status=$status disposition=$disposition source_rows=$(@($items).Count) emitted=$(@($previewRows).Count) planned_import=$plannedImportCount incomplete=$incompleteCount db_reads=False db_writes=False provider_calls=False run_id=$RunId"
