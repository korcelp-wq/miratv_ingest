...
.DESCRIPTION
    Provider inventory snapshot worker for Series streams. Larger than category snapshots, but still read-only/no DB import.
    ...
    Flow:
      ...
      3. Call provider player_api.php?action=get_series.
      4. Save raw snapshot under runtime/provider_snapshots/series_streams.
    ...
    This worker:
      - DOES call provider player_api.php for get_series only.
      - DOES NOT import to database.
      - DOES NOT call get_series.
    ...
    Intended clean-repo location:
      tools\workers\build_provider_series_streams_snapshot.ps1
#>

[CmdletBinding()]
param(
    [string]$WorkerName = "build_provider_series_streams_snapshot",
    [string]$Component = "provider_series_streams_snapshot",
    [string]$Environment = "dev",
    [string]$KillSwitchName = "ENABLE_PROVIDER_SERIES_STREAMS_SNAPSHOT",

    [int]$MacUserId = 6,
    [string]$ProviderLabel = "",
    [string]$GatewayBaseUrl = "https://miratv.club",

    [string]$SnapshotRoot = "runtime/provider_snapshots/series_streams",
    [string]$OutputRoot = "runtime/reports/provider_series_streams_snapshot",
...
function New-RunIdLocal {
    param([string]$Prefix = "provider-series-streams-snapshot")
...
function Invoke-JsonGetLocal {
    param(
        [string]$Url,
        [int]$TimeoutSec = 45,
        [string]$UserAgent = "MiraTV-MasterControl-SeriesStreamSnapshot/1.0"
    )
...
function Invoke-RawGetLocal {
    ...
    $headers = @{
        "Accept" = "application/json"
        "User-Agent" = "MiraTV-MasterControl-SeriesStreamSnapshot/1.0"
    }
...
function Get-NormalizedSeriesStreamTextLocal {
    param([string]$Text)
        ...
        $normalizedRows = @(
            $items |
                ForEach-Object {
                    ...
                    if ($_.PSObject.Properties.Name -contains "series_id") {
                        $seriesId = [string]$_.series_id
                    }
                    elseif ($_.PSObject.Properties.Name -contains "stream_id") {
                        $seriesId = [string]$_.stream_id
                    }
                    elseif ($_.PSObject.Properties.Name -contains "id") {
                        $seriesId = [string]$_.id
                    }
                    ...
                    if ($_.PSObject.Properties.Name -contains "last_modified") {
                        $lastModified = [string]$_.last_modified
                    }

                    if ($_.PSObject.Properties.Name -contains "releaseDate") {
                        $releaseDate = [string]$_.releaseDate
                    }
                    elseif ($_.PSObject.Properties.Name -contains "release_date") {
                        $releaseDate = [string]$_.release_date
                    }

                    if ($_.PSObject.Properties.Name -contains "rating") {
                    ...
                    if (-not [string]::IsNullOrWhiteSpace($seriesId) -or -not [string]::IsNullOrWhiteSpace($name)) {
                        "$seriesId|$categoryId|$lastModified|$releaseDate|$rating|$name"
                    }
                } |
                Sort-Object -Unique
        )
...
function Get-NormalizedSeriesStreamHashLocal {
    param([string]$Text)

    $normalized = Get-NormalizedSeriesStreamTextLocal -Text $Text
...
$startedAt = Get-Date
$signalName = "provider_series_streams_snapshot_completed"
...
try {
    ...
    if ($loggingAvailable) {
        ...
        if (-not $killEnabled) {
            Write-JobLog `
                ...
                -Status "blocked" `
                -Data @{
                    event_message = "Provider Series streams snapshot blocked by kill switch."
                    kill_switch_name = $KillSwitchName
                    mac_user_id = $MacUserId
                    provider_label = $provider
                } | Out-Null

            Write-Output "BLOCKED: provider Series streams snapshot blocked. run_id=$script:RunId kill_switch=$KillSwitchName"
            exit 0
        }
        ...
        Write-JobLog `
            ...
            -Status "started" `
            -Data @{
                event_message = "Provider Series streams snapshot started."
                mac_user_id = $MacUserId
                provider_label = $provider
    ...
    if (-not [bool]$credentials.username_present -or -not [bool]$credentials.password_present) {
        throw "Resolved account context did not include provider credentials required for Series stream pull. backend_credential_resolution_used=$backendCredentialResolutionUsed backend_credential_resolution_ok=$backendCredentialResolutionOk backend_error=$backendCredentialResolutionError"
    }
    ...
    if (-not $providerResult.ok) {
        throw "Provider get_series failed. url=$redactedProviderUrl status_code=$($providerResult.status_code) error=$($providerResult.error)"
    }

    if ([string]::IsNullOrWhiteSpace([string]$providerResult.body)) {
        throw "Provider get_series returned an empty body. url=$redactedProviderUrl status_code=$($providerResult.status_code)"
    }
    ...
    $snapshotPath = Join-Path $accountSnapshotDir "series_streams_$stamp.json"
    $summaryJson = Join-Path $outputRootFull "provider_series_streams_snapshot_summary_$stamp.json"
    $reportCsv = Join-Path $outputRootFull "provider_series_streams_snapshot_report_$stamp.csv"
    ...
    $normalizedSha256 = Get-NormalizedSeriesStreamHashLocal -Text ([string]$providerResult.body)
    ...
    if ($null -ne $prior) {
        ...
        $priorNormalizedHash = Get-NormalizedSeriesStreamHashLocal -Text ([string]$priorBody)
    ...
    if ($loggingAvailable) {
        ...
        Write-JobLog `
            ...
            -Status $statusValue `
            -Data @{
                event_message = "Provider Series streams snapshot completed."
                provider_calls = $true
                db_imported = $false
                mac_user_id = $MacUserId
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/build_provider_series_streams_snapshot.ps1" `
            -Data @{
                dashboard_panel = "Provider Sync"
                widget_key = "provider.series.streams.snapshot"
                owner = "Content Ops"
                kill_switch_name = $KillSwitchName
                provider_calls = $true
                db_imported = $false
    ...
    Write-Output ("OK: provider Series streams snapshot completed. status=pass provider_calls=True db_imported=False mac_user_id={0} provider_label=""{1}"" action=get_series item_count_estimate={2} change_status={3} changed={4} backend_credential_resolution_used={5} output_root=""{6}"" run_id={7}" -f `
        $MacUserId, `
        $resolvedProvider, `
        $itemCount, `
        $changeStatus, `
    ...
    if ($loggingAvailable) {
        Write-JobLog `
            ...
            -Status "failed" `
            -Data @{
                event_message = "Provider Series streams snapshot failed."
                error = $errorMessage
                mac_user_id = $MacUserId
    ...
    Write-Error "FAILED: provider Series streams snapshot failed. run_id=$script:RunId $errorMessage"
    exit 1
}
