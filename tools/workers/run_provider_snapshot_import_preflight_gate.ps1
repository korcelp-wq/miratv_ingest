...
.DESCRIPTION
  No-write, no-provider-call gate runner for the provider snapshot import planning stack.

  Runs only report/planning workers:
    - plan_provider_snapshot_import_preview.ps1
    - import_provider_snapshot_delta_dryrun.ps1
    - import_vod_streams_delta_preview.ps1
    - summarize_provider_snapshot_import_readiness.ps1
    - plan_provider_snapshot_import_execution.ps1

  It does not run the provider snapshot spine.
...
$WorkerName = "run_provider_snapshot_import_preflight_gate"
$Component = "provider_snapshot_import_preflight_gate"
$DatabaseTarget = "none"
$SourceName = "runtime_reports"
$KillSwitchName = "ENABLE_PROVIDER_SNAPSHOT_IMPORT_PREFLIGHT_GATE"

$CompletedSignal = "provider_snapshot_import_preflight_gate_completed"
$PassCountSignal = "provider_snapshot_import_preflight_gate_pass_count"
$FailCountSignal = "provider_snapshot_import_preflight_gate_fail_count"
$StepCountSignal = "provider_snapshot_import_preflight_gate_step_count"

$StartedAt = Get-Date
$RunId = "$WorkerName-$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssfffZ'))-$([guid]::NewGuid().ToString('N').Substring(0, 16))"
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$OutputRoot = Join-Path $RepoRoot "runtime\reports\provider_snapshot_import_preflight_gate"
$LogRoot = Join-Path $RepoRoot "runtime\logs\provider_snapshot_import_preflight_gate"
...
try {
    ...
    $steps = @(
        [pscustomobject]@{ Order = 1; Name = "plan_provider_snapshot_import_preview"; Path = "tools\workers\plan_provider_snapshot_import_preview.ps1" },
        [pscustomobject]@{ Order = 2; Name = "import_provider_snapshot_delta_dryrun"; Path = "tools\workers\import_provider_snapshot_delta_dryrun.ps1" },
        [pscustomobject]@{ Order = 3; Name = "import_vod_streams_delta_preview"; Path = "tools\workers\import_vod_streams_delta_preview.ps1" },
        [pscustomobject]@{ Order = 4; Name = "summarize_provider_snapshot_import_readiness"; Path = "tools\workers\summarize_provider_snapshot_import_readiness.ps1" },
    ...
    $disposition = if ($overallStatus -eq "pass") { "preflight_gate_passed" } else { "preflight_gate_failed" }

    $timestamp = (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
    $reportCsv = Join-Path $OutputRoot "provider_snapshot_import_preflight_gate_report_$timestamp.csv"
    $summaryJson = Join-Path $OutputRoot "provider_snapshot_import_preflight_gate_summary_$timestamp.json"
    ...
    if (-not $Quiet) {
        Write-Output "OK: provider snapshot import preflight gate completed. status=$overallStatus disposition=$disposition step_count=$stepCount pass_count=$passCount fail_count=$failCount db_writes=False provider_calls=False run_id=$RunId"
        Write-Output "FILES: report_csv=$reportCsv summary_json=$summaryJson"
        $results | Format-Table step_order, step_name, status, exit_code, duration_ms -AutoSize
    }
    ...
    Write-Error "FAILED: provider snapshot import preflight gate failed. $message run_id=$RunId"
    exit 1
}
