...
.DESCRIPTION
    ...
    Flow:
      ...
      3. Call provider player_api.php?action=get_live_categories.
      4. Save raw snapshot under runtime/provider_snapshots/live_categories.
    ...
    This worker:
      - DOES call provider player_api.php for get_live_categories only.
    ...
    Intended clean-repo location:
      tools\workers\build_provider_live_categories_snapshot.ps1
#>

[CmdletBinding()]
param(
    [string]$WorkerName = "build_provider_live_categories_snapshot",
    [string]$Component = "provider_live_categories_snapshot",
    [string]$Environment = "dev",
    [string]$KillSwitchName = "ENABLE_PROVIDER_LIVE_CATEGORIES_SNAPSHOT",

    [int]$MacUserId = 6,
    [string]$ProviderLabel = "",
    [string]$GatewayBaseUrl = "https://miratv.club",

    [string]$SnapshotRoot = "runtime/provider_snapshots/live_categories",
    [string]$OutputRoot = "runtime/reports/provider_live_categories_snapshot",
...
function New-RunIdLocal {
    param([string]$Prefix = "provider-live-categories-snapshot")
...
function Invoke-JsonGetLocal {
    param(
        [string]$Url,
        [int]$TimeoutSec = 45,
        [string]$UserAgent = "MiraTV-MasterControl-LiveCategorySnapshot/1.0"
    )
...
function Invoke-RawGetLocal {
    ...
    $headers = @{
        "Accept" = "application/json"
        "User-Agent" = "MiraTV-MasterControl-LiveCategorySnapshot/1.0"
    }
...
$startedAt = Get-Date
$signalName = "provider_live_categories_snapshot_completed"
...
try {
    ...
    if ($loggingAvailable) {
        ...
        if (-not $killEnabled) {
            Write-JobLog `
                ...
                -Status "blocked" `
                -Data @{
                    event_message = "Provider live categories snapshot blocked by kill switch."
                    kill_switch_name = $KillSwitchName
                    mac_user_id = $MacUserId
                    provider_label = $provider
                } | Out-Null

            Write-Output "BLOCKED: provider live categories snapshot blocked. run_id=$script:RunId kill_switch=$KillSwitchName"
            exit 0
        }
        ...
        Write-JobLog `
            ...
            -Status "started" `
            -Data @{
                event_message = "Provider live categories snapshot started."
                mac_user_id = $MacUserId
                provider_label = $provider
    ...
    if (-not [bool]$credentials.username_present -or -not [bool]$credentials.password_present) {
        throw "Resolved account context did not include provider credentials required for live category pull. backend_credential_resolution_used=$backendCredentialResolutionUsed backend_credential_resolution_ok=$backendCredentialResolutionOk backend_error=$backendCredentialResolutionError"
    }
    ...
    if (-not $providerResult.ok) {
        throw "Provider get_live_categories failed. url=$redactedProviderUrl status_code=$($providerResult.status_code) error=$($providerResult.error)"
    }

    if ([string]::IsNullOrWhiteSpace([string]$providerResult.body)) {
        throw "Provider get_live_categories returned an empty body. url=$redactedProviderUrl status_code=$($providerResult.status_code)"
    }
    ...
    $snapshotPath = Join-Path $accountSnapshotDir "live_categories_$stamp.json"
    $summaryJson = Join-Path $outputRootFull "provider_live_categories_snapshot_summary_$stamp.json"
    $reportCsv = Join-Path $outputRootFull "provider_live_categories_snapshot_report_$stamp.csv"
    ...
    if ($loggingAvailable) {
        ...
        Write-JobLog `
            ...
            -Status $statusValue `
            -Data @{
                event_message = "Provider live categories snapshot completed."
                provider_calls = $true
                db_imported = $false
                mac_user_id = $MacUserId
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/build_provider_live_categories_snapshot.ps1" `
            -Data @{
                dashboard_panel = "Provider Sync"
                widget_key = "provider.live.categories.snapshot"
                owner = "Content Ops"
                kill_switch_name = $KillSwitchName
                provider_calls = $true
                db_imported = $false
    ...
    Write-Output ("OK: provider live categories snapshot completed. status=pass provider_calls=True db_imported=False mac_user_id={0} provider_label=""{1}"" action=get_live_categories item_count_estimate={2} change_status={3} changed={4} backend_credential_resolution_used={5} output_root=""{6}"" run_id={7}" -f `
        $MacUserId, `
        $resolvedProvider, `
        $itemCount, `
        $changeStatus, `
    ...
    if ($loggingAvailable) {
        Write-JobLog `
            ...
            -Status "failed" `
            -Data @{
                event_message = "Provider live categories snapshot failed."
                error = $errorMessage
                mac_user_id = $MacUserId
