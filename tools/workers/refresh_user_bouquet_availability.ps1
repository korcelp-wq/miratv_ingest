# MiraTV User Bouquet Availability Refresh Worker
# File: tools/workers/refresh_user_bouquet_availability.ps1
# Purpose:
#   P0.1 availability refresh lane scaffold.
#   Establishes the automation contract for user/provider bouquet availability refresh.
#
...
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File "tools/workers/refresh_user_bouquet_availability.ps1" -Environment "dev"
#
# Optional snapshot input:
#   powershell -NoProfile -ExecutionPolicy Bypass -File "tools/workers/refresh_user_bouquet_availability.ps1" `
#     -Environment "dev" `
#     -Mode "SnapshotInput" `
#     -InputJsonPath "runtime/samples/bouquet_availability_snapshot.json"

[CmdletBinding()]
param(
    [string]$WorkerName = "availability_worker",
    [string]$Component = "availability_worker",
...
function Get-SnapshotItems {
    ...
    if ($Snapshot.PSObject.Properties.Name -contains "bouquets") {
        return Convert-ToArray -Value $Snapshot.bouquets
    }
...
function Get-AvailabilityMetrics {
    ...
    foreach ($item in $Items) {
        $statusRaw = Get-PropertyValue -Object $item -Names @("status", "availability_status", "state", "is_available")
        $status = ""
        ...
        if ($status -in @("1", "true", "available", "active", "ok")) {
            $available++
        }
        elseif ($status -in @("0", "false", "unavailable", "inactive", "disabled", "denied")) {
            $unavailable++
        }
...
$script:RunId = New-RunId -Prefix "availability-refresh"
...
try {
    ...
    if (-not $enabled) {
        Write-JobLog `
            -RunId $script:RunId `
            -JobName "refresh_user_bouquet_availability" `
            -WorkerName $WorkerName `
            -Component $Component `
            -Environment $Environment `
            -Status "job_skipped" `
            -EventType "job_skipped" `
            -SourceName "provider_bouquet_availability" `
            -DurationMs (Get-DurationMs -Start $script:StartedAt) `
            -Data @{
                kill_switch_name = $KillSwitchName
                kill_switch_enabled = $false
                reason = "availability refresh disabled by kill switch"
                mode = $Mode
            } `
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/refresh_user_bouquet_availability.ps1" `
            -Data @{
    ...
    Write-Output "OK: availability refresh scaffold completed. status=$status mode=$Mode total=$($metrics.total) lag_minutes=$refreshLagMinutes run_id=$script:RunId"
    exit 0
}
catch {
    ...
    if ([string]::IsNullOrWhiteSpace($script:RunId)) {
        $script:RunId = "availability-refresh-failed-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssfffZ")
    }
    ...
    try {
        Write-JobLog `
            ...
            -SourceName "provider_bouquet_availability" `
            -DurationMs $duration `
            -ErrorCode "AVAILABILITY_REFRESH_FAILED" `
        ...
        Write-Error "Availability worker failed and failed to log error: $($_.Exception.Message)"
    }

    Write-Error "FAILED: availability refresh worker failed. run_id=$script:RunId error=$message"
    exit 1
}