# MiraTV User Series Availability Refresh Worker
# File: tools/workers/refresh_user_series_availability.ps1
# Purpose:
#   P0.1 availability refresh lane scaffold for user/provider series availability.
#   Establishes the automation contract for series-level entitlement/access refresh.
#
...
#   pwsh -NoProfile -ExecutionPolicy Bypass -File "tools/workers/refresh_user_series_availability.ps1" `
#     -Environment "dev" `
#     -Mode "SnapshotInput" `
#     -InputJsonPath "runtime/samples/series_availability_snapshot.json"

[CmdletBinding()]
param(
    [string]$WorkerName = "availability_series_worker",
    [string]$Component = "availability_series_worker",
...
function Get-SnapshotItems {
    ...
    if ($Snapshot.PSObject.Properties.Name -contains "series") {
        return Convert-ToArray -Value $Snapshot.series
    }

    if ($Snapshot.PSObject.Properties.Name -contains "availability_series") {
        return Convert-ToArray -Value $Snapshot.availability_series
    }

    if ($Snapshot.PSObject.Properties.Name -contains "series_availability") {
        return Convert-ToArray -Value $Snapshot.series_availability
    }
...
function Get-SeriesAvailabilityMetrics {
    [CmdletBinding()]
    param(
        [object[]]$Items
    )
    ...
    foreach ($item in $Items) {
        ...
        $hasSeriesInfoRaw = Get-PropertyValue -Object $item -Names @("has_series_info", "series_info_loaded", "has_info")
        $hasSeriesInfo = ""

        if ($null -ne $hasSeriesInfoRaw) {
            $hasSeriesInfo = ([string]$hasSeriesInfoRaw).Trim().ToLowerInvariant()
        }

        if ($hasSeriesInfo -in @("1", "true", "yes", "y", "loaded", "ok")) {
            $withSeriesInfo++
        }
        elseif ($hasSeriesInfo -in @("0", "false", "no", "n", "missing", "not_loaded")) {
            $missingSeriesInfo++
        }

        $seasonRaw = Get-PropertyValue -Object $item -Names @("season_count", "seasons", "total_seasons")
        $episodeRaw = Get-PropertyValue -Object $item -Names @("episode_count", "episodes", "total_episodes")

        $seasonValue = 0
        $episodeValue = 0

        if ($null -ne $seasonRaw -and [int]::TryParse([string]$seasonRaw, [ref]$seasonValue)) {
            $seasonCount += $seasonValue
        }

        if ($null -ne $episodeRaw -and [int]::TryParse([string]$episodeRaw, [ref]$episodeValue)) {
            $episodeCount += $episodeValue
        }
    }
    ...
    return [ordered]@{
        ...
        stale = $stale
        unknown = $unknown
        with_series_info = $withSeriesInfo
        missing_series_info = $missingSeriesInfo
...
$script:RunId = New-RunId -Prefix "series-availability-refresh"
...
try {
    ...
    if (-not $enabled) {
        Write-JobLog `
            -RunId $script:RunId `
            -JobName "refresh_user_series_availability" `
            -WorkerName $WorkerName `
            -Component $Component `
            -Environment $Environment `
            -Status "job_skipped" `
            -EventType "job_skipped" `
            -SourceName "provider_series_availability" `
            -DurationMs (Get-DurationMs -Start $script:StartedAt) `
            -Data @{
                kill_switch_name = $KillSwitchName
                kill_switch_enabled = $false
                reason = "series availability refresh disabled by kill switch"
                mode = $Mode
            } `
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/refresh_user_series_availability.ps1" `
            -Data @{
                dashboard_panel = "Availability"
                widget_key = "availability.refresh.status"
                owner = "IP Ops"
                kill_switch_name = $KillSwitchName
                availability_scope = "series"
            } `
            -LogRoot $LogRoot | Out-Null
    ...
    $metrics = Get-SeriesAvailabilityMetrics -Items $items
    ...
    Emit-Signal `
        ...
        -Data @{
            ...
            with_series_info = $metrics.with_series_info
            missing_series_info = $metrics.missing_series_info
            season_count = $metrics.season_count
    ...
    Write-Output "OK: series availability refresh scaffold completed. status=$status mode=$Mode total=$($metrics.total) lag_minutes=$refreshLagMinutes run_id=$script:RunId"
    exit 0
}
catch {
    ...
    if ([string]::IsNullOrWhiteSpace($script:RunId)) {
        $script:RunId = "series-availability-refresh-failed-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssfffZ")
    }
    ...
    try {
        Write-JobLog `
            ...
            -SourceName "provider_series_availability" `
            -DurationMs $duration `
            -ErrorCode "SERIES_AVAILABILITY_REFRESH_FAILED" `
        ...
        Write-Error "Series availability worker failed and failed to log error: $($_.Exception.Message)"
    }

    Write-Error "FAILED: series availability refresh worker failed. run_id=$script:RunId error=$message"
    exit 1
}