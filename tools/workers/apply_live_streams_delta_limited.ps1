<#
.SYNOPSIS
  Apply Live streams delta with strict bounded controls.

.DESCRIPTION
  Governed Live streams limited apply worker.
  ...
  Current behavior:
    ...
    - A real write request requires all of the following:
        -Apply
        -AllowDbWrite
        ENABLE_LIVE_STREAMS_DELTA_LIMITED_APPLY_WRITES=true
        -WriteAuthorizationCode "APPLY_LIVE_STREAMS_DELTA_LIMITED"
    - The worker self-blocks unless the latest real selector says:
        candidate_found=True
        selected_lane=live_streams
        next_worker=apply_live_streams_delta_limited.ps1
...
$WorkerName = "apply_live_streams_delta_limited"
$Component = "live_streams_delta_limited_apply"
$DatabaseTarget = "xpdgxfsp_content"
$SourceName = "provider_snapshot_import_candidate_selector"
$KillSwitchName = "ENABLE_LIVE_STREAMS_DELTA_LIMITED_APPLY"
$WriteKillSwitchName = "ENABLE_LIVE_STREAMS_DELTA_LIMITED_APPLY_WRITES"
$ExpectedWriteAuthorizationCode = "APPLY_LIVE_STREAMS_DELTA_LIMITED"

$CompletedSignal = "live_streams_delta_limited_apply_completed"
$DispositionSignal = "live_streams_delta_limited_apply_disposition"
$WouldWriteCountSignal = "live_streams_delta_limited_apply_would_write_count"
$ActualWriteCountSignal = "live_streams_delta_limited_apply_actual_write_count"

$StartedAt = Get-Date
$RunId = "$WorkerName-$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssfffZ'))-$([guid]::NewGuid().ToString('N').Substring(0, 16))"
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$OutputRoot = Join-Path $RepoRoot "runtime\reports\live_streams_delta_limited_apply"
$LogRoot = Join-Path $RepoRoot "runtime\logs\live_streams_delta_limited_apply"
...
function Get-LatestVodPreviewSummary {
    $latest = Get-LatestFile -Folder (Join-Path $RepoRoot "runtime\reports\live_streams_delta_import_preview") -Filter "live_streams_delta_import_preview_summary_*.json"
    if ($null -eq $latest) { return $null }
    return Read-JsonFile -Path $latest.FullName
}

function Get-LatestSqlContractSummary {
    $latest = Get-LatestFile -Folder (Join-Path $RepoRoot "runtime\reports\live_streams_apply_sql_contract") -Filter "live_streams_apply_sql_contract_summary_*.json"
    if ($null -eq $latest) { return $null }
    return Read-JsonFile -Path $latest.FullName
}
...
try {
    ...
    $livePreview = Get-LatestVodPreviewSummary
    ...
    $plannedImportCount = Get-IntValue -Object $livePreview -Name "planned_import_count" -Default 0
    $sourceRowCount = Get-IntValue -Object $livePreview -Name "source_row_count" -Default 0
    $manualReviewCount = Get-IntValue -Object $livePreview -Name "manual_review_count" -Default 0
    $skippedProviderNoiseCount = Get-IntValue -Object $livePreview -Name "skipped_provider_noise_count" -Default 0
    $livePreviewOutputCsv = Get-Text -Object $livePreview -Name "output_csv" -Default ""
    ...
    if ($selectedLane -ne "live_streams") { $blockReasons += "real_selector_selected_lane_not_live_streams" }
    if ($selectorNextWorker -ne "apply_live_streams_delta_limited.ps1") { $blockReasons += "real_selector_next_worker_not_this_worker" }
    if ($plannedImportCount -le 0) { $blockReasons += "live_preview_planned_import_count_zero" }
    if ($manualReviewCount -gt 0) { $blockReasons += "live_preview_manual_review_count_gt_zero" }
    if ([string]::IsNullOrWhiteSpace($livePreviewOutputCsv) -or -not (Test-Path -LiteralPath $livePreviewOutputCsv)) { $blockReasons += "live_preview_output_csv_missing" }
    ...
    $reportCsv = Join-Path $OutputRoot "live_streams_delta_limited_apply_$timestamp.csv"
    $reportJson = Join-Path $OutputRoot "live_streams_delta_limited_apply_$timestamp.json"
    $summaryJson = Join-Path $OutputRoot "live_streams_delta_limited_apply_summary_$timestamp.json"
