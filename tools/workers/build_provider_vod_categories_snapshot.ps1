...
.DESCRIPTION
    First low-risk provider replacement worker.
    ...
    Flow:
      ...
      3. Call provider player_api.php?action=get_vod_categories.
      4. Save raw snapshot under runtime/provider_snapshots/vod_categories.
    ...
    This worker:
      - DOES call provider player_api.php for get_vod_categories only.
    ...
    Intended clean-repo location:
      tools\workers\build_provider_vod_categories_snapshot.ps1
#>

[CmdletBinding()]
param(
    [string]$WorkerName = "build_provider_vod_categories_snapshot",
    [string]$Component = "provider_vod_categories_snapshot",
    [string]$Environment = "dev",
    [string]$KillSwitchName = "ENABLE_PROVIDER_VOD_CATEGORIES_SNAPSHOT",

    [int]$MacUserId = 6,
    [string]$ProviderLabel = "",
    [string]$GatewayBaseUrl = "https://miratv.club",

    [string]$SnapshotRoot = "runtime/provider_snapshots/vod_categories",
    [string]$OutputRoot = "runtime/reports/provider_vod_categories_snapshot",
...
function New-RunIdLocal {
    param([string]$Prefix = "provider-vod-categories-snapshot")
...
function Invoke-JsonGetLocal {
    param(
        [string]$Url,
        [int]$TimeoutSec = 45,
        [string]$UserAgent = "MiraTV-MasterControl-VodCategorySnapshot/1.0"
    )
...
function Invoke-RawGetLocal {
    ...
    $headers = @{
        "Accept" = "application/json"
        "User-Agent" = "MiraTV-MasterControl-VodCategorySnapshot/1.0"
    }
...
function Get-NormalizedVodCategoryTextLocal {
    param([string]$Text)
...
function Get-NormalizedVodCategoryHashLocal {
    param([string]$Text)

    $normalized = Get-NormalizedVodCategoryTextLocal -Text $Text
...
$startedAt = Get-Date
$signalName = "provider_vod_categories_snapshot_completed"
...
try {
    ...
    if ($loggingAvailable) {
        ...
        if (-not $killEnabled) {
            Write-JobLog `
                ...
                -Status "blocked" `
                -Data @{
                    event_message = "Provider VOD categories snapshot blocked by kill switch."
                    kill_switch_name = $KillSwitchName
                    mac_user_id = $MacUserId
                    provider_label = $provider
                } | Out-Null

            Write-Output "BLOCKED: provider VOD categories snapshot blocked. run_id=$script:RunId kill_switch=$KillSwitchName"
            exit 0
        }
        ...
        Write-JobLog `
            ...
            -Status "started" `
            -Data @{
                event_message = "Provider VOD categories snapshot started."
                mac_user_id = $MacUserId
                provider_label = $provider
    ...
    if (-not [bool]$credentials.username_present -or -not [bool]$credentials.password_present) {
        throw "Resolved account context did not include provider credentials required for VOD category pull. backend_credential_resolution_used=$backendCredentialResolutionUsed backend_credential_resolution_ok=$backendCredentialResolutionOk backend_error=$backendCredentialResolutionError"
    }

    $script:Stage = "call_provider_vod_categories"
    ...
    if (-not $providerResult.ok) {
        throw "Provider get_vod_categories failed. url=$redactedProviderUrl status_code=$($providerResult.status_code) error=$($providerResult.error)"
    }

    if ([string]::IsNullOrWhiteSpace([string]$providerResult.body)) {
        throw "Provider get_vod_categories returned an empty body. url=$redactedProviderUrl status_code=$($providerResult.status_code)"
    }
    ...
    $snapshotPath = Join-Path $accountSnapshotDir "vod_categories_$stamp.json"
    $summaryJson = Join-Path $outputRootFull "provider_vod_categories_snapshot_summary_$stamp.json"
    $reportCsv = Join-Path $outputRootFull "provider_vod_categories_snapshot_report_$stamp.csv"
    ...
    $normalizedSha256 = Get-NormalizedVodCategoryHashLocal -Text ([string]$providerResult.body)
    ...
    if ($null -ne $prior) {
        ...
        $priorNormalizedHash = Get-NormalizedVodCategoryHashLocal -Text ([string]$priorBody)
    ...
    if ($loggingAvailable) {
        ...
        Write-JobLog `
            ...
            -Status $statusValue `
            -Data @{
                event_message = "Provider VOD categories snapshot completed."
                provider_calls = $true
                db_imported = $false
                mac_user_id = $MacUserId
        ...
        Emit-Signal `
            ...
            -SourceTableOrEndpoint "tools/workers/build_provider_vod_categories_snapshot.ps1" `
            -Data @{
                dashboard_panel = "Provider Sync"
                widget_key = "provider.vod.categories.snapshot"
                owner = "Content Ops"
                kill_switch_name = $KillSwitchName
                provider_calls = $true
                db_imported = $false
    ...
    Write-Output ("OK: provider VOD categories snapshot completed. status=pass provider_calls=True db_imported=False mac_user_id={0} provider_label=""{1}"" action=get_vod_categories item_count_estimate={2} change_status={3} changed={4} backend_credential_resolution_used={5} output_root=""{6}"" run_id={7}" -f `
        $MacUserId, `
        $resolvedProvider, `
        $itemCount, `
        $changeStatus, `
    ...
    if ($loggingAvailable) {
        Write-JobLog `
            ...
            -Status "failed" `
            -Data @{
                event_message = "Provider VOD categories snapshot failed."
                error = $errorMessage
                mac_user_id = $MacUserId
