...
try {
    ...
    if (@($blockReasons).Count -eq 0) {
        if ($Apply -and -not $writeAuthorization.authorized) {
            ...
            $candidateRows = @(Import-Csv -LiteralPath $vodPreviewOutputCsv | Where-Object { [string]::IsNullOrWhiteSpace((Get-Text -Object $_ -Name "import_status" -Default "")) } | Select-Object -Skip $Offset -First $Limit)
