...
function Invoke-QueryRows {
    ...
    try {
        ...
        return @(Convert-ToRowArray -InputObject $rows)
    }
    catch {
...
function Convert-ToRowArray {
    ...
    if ($InputObject -is [System.Array]) {
        ...
        foreach ($item in $InputObject) {
            $rows += @(Convert-ToRowArray -InputObject $item)
        }
        return @($rows)
    }
    ...
    if ($InputObject -is [PSCustomObject]) {
        ...
        if ($propNames -contains 'rows' -and $null -ne $InputObject.rows) {
            return @(Convert-ToRowArray -InputObject $InputObject.rows)
        }

        if ($propNames -contains 'data' -and $null -ne $InputObject.data) {
            return @(Convert-ToRowArray -InputObject $InputObject.data)
        }

        if ($propNames -contains 'result' -and $null -ne $InputObject.result) {
            return @(Convert-ToRowArray -InputObject $InputObject.result)
        }
...
function Get-RowPropertyValue {
    ...
    foreach ($name in $CandidateNames) {
        try {
            if ($Row -is [PSCustomObject] -and $Row.PSObject.Properties.Name -contains $name) {
                $value = $Row.$name
...
foreach ($seed in $seeds) {
    $seedQuery = [string](Get-RowPropertyValue -Row $seed -CandidateNames @('slot_value','slotValue','value','slotvalue') -Default '')
    if ([string]::IsNullOrWhiteSpace($seedQuery)) {
        if ($VerboseLoop) {
            $available = if ($seed -is [PSCustomObject]) { ($seed.PSObject.Properties.Name -join ', ') } else { $seed.GetType().FullName }
            Write-Info ("Skipping seed with no slot_value-compatible property. Available: {0}" -f $available) Yellow
        }
        continue
    }
