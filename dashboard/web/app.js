...
async function loadStatus() {
  try {
    ...
    setText('ollamaModels', asArray(ollama.models).join(', ') || ollama.message || '-');

    const serviceRows = asArray(data.services).map(s => statusRow([s.name, s.state, s.count ? `${s.count} proc` : '0 proc', s.pid ? `PID ${s.pid}` : '-']));
    renderTable($('servicesTable'), serviceRows, 'No service state available');

    const cviRows = asArray(data.cvi).map(c => statusRow([c.name || c.procedure, c.state, c.row_count == null ? '- rows' : `${c.row_count} rows`, c.response_ms == null ? c.message : `${c.response_ms} ms`]));
    renderTable($('cviTable'), cviRows, 'No CVI checks configured');

    const errorRows = asArray(data.error_rates).map(e => {
      const div = document.createElement('div');
      div.className = 'row';
    ...
    const log = $('logFeed');
    log.innerHTML = '';
    asArray(data.recent_log).slice(-80).reverse().forEach(l => {
      const div = document.createElement('div');
