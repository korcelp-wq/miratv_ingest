...
function Get-ServiceStates {
    $patterns = @(
        @{ name = 'Spine Scheduler'; pattern = 'C:\miratv_ingest\workers\spine\spine_scheduler_total.ps1' },
        @{ name = 'CVI Watcher'; pattern = 'C:\miratv_ingest\watcher_cvi.ps1' },
        @{ name = 'Telemetry Watcher'; pattern = 'C:\miratv_ingest\workers\telemetry_watcher.ps1' },
        @{ name = 'Spool Uploader'; pattern = 'C:\miratv_ingest\spool_uploader.ps1' },
        @{ name = 'AI Learning Loop'; pattern = 'C:\miratv_ingest\workers\GovernanceLearner.ps1' },
        @{ name = 'Knowledge Miner'; pattern = 'C:\miratv_ingest\workers\KnowledgeMiner.ps1' },
        @{ name = 'Accessory Upload Loop'; pattern = 'C:\miratv_ingest\MASTER_ACCESSORY_UPLOAD_LOOP.bat' },
        @{ name = 'Runner Loop'; pattern = 'C:\miratv_ingest\master_runner_loop.bat' },
        @{ name = 'BGC Watcher Service'; pattern = 'C:\MiraTV\Modules\IMG\BGC\BGC_Runtime_Watcher_Service.ps1' }
    )
    return @($patterns | ForEach-Object { Get-ProcessStateByPattern -Name $_.name -Pattern $_.pattern })
}
