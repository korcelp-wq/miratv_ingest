#!/usr/bin/env pwsh
# Auto Ingester - Finds and registers all script files (PS1, BAT, PHP)
...
$directories = @(
    ...
    "C:\Users\Korce\Downloads\public_html (2)", #backend export of www.miratv.club/
    "C:\MIRATV" #miraTV
)
...
foreach ($dir in $directories) {
    if (Test-Path $dir) {
        foreach ($ext in $fileTypes.Keys) {
            ...
            $allFiles[$ext] += $files
            Write-Host "  Found $($files.Count) *$ext files in $dir" -ForegroundColor Gray
        }
    }
}
...
$totalFiles = ($allFiles.Values | Measure-Object -Property Count -Sum).Sum
...
foreach ($ext in $fileTypes.Keys) {
    ...
    foreach ($file in $files) {
        ...
        Write-Progress -Activity "Processing $($typeInfo.type) Files" -Status $fileName -PercentComplete (($processed / $totalFiles) * 100)
        
        try {
            # Read file content (limit to first 10KB for performance)
            $content = ""
            if (Test-Path $filePath) {
                $content = Get-Content $filePath -Raw -TotalCount 100 -ErrorAction Stop
            }
...
VALUES 
('$escapedName', '$domain', '$scriptType', '$($typeInfo.system)', '$escapedPath', '$(if ($escapedDesc) { $escapedDesc } else { "No description" })', 'Auto-discovered during file scan', 1, NOW())
"@
...
foreach ($group in $fileGroups.Keys) {
    ...
    if ($files.Count -gt 1) {
        ...
        foreach ($fileInfo in $files) {
            $getIdSql = "SELECT procedure_id FROM pcde_procedure_registry WHERE source_path = '$($fileInfo.Path)'"
...
foreach ($ext in $fileTypes.Keys) {
    $type = $fileTypes[$ext].type
    $count = ($allFiles[$ext] | Measure-Object).Count
    Write-Host "  $type ($ext): $count files" -ForegroundColor White
}
Write-Host "="*60
