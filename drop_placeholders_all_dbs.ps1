# MiraTV: Drop placeholder routines in all databases using short names
...
foreach ($db in $dbs) {
  foreach ($routine in $routines) {
    $sql = "DROP PROCEDURE IF EXISTS $routine;"
    try {
      .\Query.ps1 -Db $db -Sql $sql
      Write-Host "[$db] $routine dropped" -ForegroundColor Green
    } catch {
      Write-Host "[$db] $routine drop failed: $_" -ForegroundColor Yellow
    }
  }
}