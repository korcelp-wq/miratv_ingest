...
$config = @{
    ...
    BGC = @{
        ...
        ControlPanelScript   = 'C:\MiraTV\Modules\IMG\BGC\bin\ui\BGC_Control_Panel.ps1'
        HistoryBrowserScript = 'C:\MiraTV\Modules\IMG\BGC\bin\ui\BGC_History_Browser.ps1'
        WatcherWindowScript  = 'C:\MiraTV\Modules\IMG\BGC\bin\ui\BGC_Runtime_Watcher.ps1'
        WatcherServiceScript = 'C:\MiraTV\Modules\IMG\BGC\bin\BGC_Runtime_Watcher_Service.ps1'
...
function Write-ServiceStatusGroup {
    param(
        [string]$Title,
        [string[]]$ServiceNames,
    ...
    foreach ($service in $ServiceNames) {
        ...
        $label = if ($info -and $info.name) { $info.name } else { $service }
        $menu = if ($info -and $info.menu) { $info.menu } else { '--' }

        if ($job -and $job.State -eq 'Running') {
            Write-Host ("    [OK]    [{0}] {1}" -f $menu, $label) -ForegroundColor Green
        }
        elseif ($job) {
            Write-Host ("    [WARN]  [{0}] {1} ({2})" -f $menu, $label, $job.State) -ForegroundColor Yellow
        }
        else {
            Write-Host ("    [PAUSE] [{0}] {1}" -f $menu, $label) -ForegroundColor Gray
        }
    }
}

function Write-MonitoredExternalStatus {
    Write-Host ""
    Write-Host "  MONITORED EXTERNAL / ADJACENT SERVICES:" -ForegroundColor Cyan

    if ($config.BGC.Enabled) {
        $bgcStatus = Get-BgcWatcherStatus
        if ($bgcStatus.Running) {
            Write-Host ("    [OK]    BGC Watcher Service: {0}" -f $bgcStatus.State) -ForegroundColor Green
        }
        else {
            Write-Host ("    [PAUSE] BGC Watcher Service: {0}" -f $bgcStatus.State) -ForegroundColor Gray
        }
        ...
        if ($bgcStatus.LastAction) {
            Write-Host ("           Last Action: {0}" -f $bgcStatus.LastAction) -ForegroundColor DarkGray
        }
    }

    try {
        $dash = @(Get-CVIWebDashboardProcesses)
        if ($dash.Count -gt 0) {
            Write-Host ("    [OK]    CVI Web Dashboard: RUNNING ({0} proc)" -f $dash.Count) -ForegroundColor Green
        }
        else {
            Write-Host "    [PAUSE] CVI Web Dashboard: STOPPED" -ForegroundColor Gray
        }
    }
    catch {
        Write-Host "    [WARN]  CVI Web Dashboard: UNKNOWN" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        if ($ollamaTest.models) {
            ...
            Write-Host ("    [OK]    Ollama Local AI: CONNECTED ({0})" -f ($modelNames -join ', ')) -ForegroundColor Green
        }
        else {
            Write-Host "    [WARN]  Ollama Local AI: CONNECTED / NO MODELS LISTED" -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "    [PAUSE] Ollama Local AI: NOT RUNNING / UNAVAILABLE" -ForegroundColor Gray
    }
}
...
function Show-Status {
    ...
    $serviceMap = @{
        ...
        'MasteryAccessoryLoop' = @{ name = 'Mastery Accessory Loop'; menu = '17' }
        'MainSeriesLoop'       = @{ name = 'Main Series Loop'; menu = '18' }
        'MasterUploadLoop'     = @{ name = 'Master Upload Loop'; menu = '19' }
    }
    
    Write-Host "ACTIVE MANAGED SERVICES" -ForegroundColor Yellow
    Write-ServiceStatusGroup -Title "  CORE SERVICES (4-8):" -ServiceNames @('SpineScheduler', 'CVIWatcher', 'TelemetryWatcher', 'SpoolUploader', 'AILearning') -ServiceMap $serviceMap
    Write-ServiceStatusGroup -Title "  LEARNING LOOP SERVICES (9-11):" -ServiceNames @('LearningLoop', 'AccessoryLoop', 'RunnerLoop') -ServiceMap $serviceMap
    Write-ServiceStatusGroup -Title "  ADDITIONAL MANAGED SERVICES (17-19):" -ServiceNames @('MasteryAccessoryLoop', 'MainSeriesLoop', 'MasterUploadLoop') -ServiceMap $serviceMap
    Write-MonitoredExternalStatus
...
function Show-Menu {
    ...
    Write-Host ""
    Write-Host "  ADDITIONAL MANAGED SERVICES:" -ForegroundColor White
    Write-Host "   17 Start Mastery Accessory Loop"
    Write-Host "   18 Start Main Series Loop"
    Write-Host "   19 Start Master Upload Loop"
    Write-Host "   38 Stop Mastery Accessory Loop"
    Write-Host "   39 Stop Main Series Loop"
    Write-Host "   40 Stop Master Upload Loop"
    Write-Host ""
    Write-Host "  MONITORED EXTERNAL / ADJACENT SERVICES:" -ForegroundColor White
    Write-Host "   Displayed in status/dashboard only unless a dedicated interface exists"
    Write-Host "   BGC Watcher Service"
    Write-Host "   CVI Web Dashboard"
    Write-Host "   Ollama Local AI"
    Write-Host ""
    Write-Host "  ADDITIONAL TOOLS:" -ForegroundColor White
...
function Start-Service {
    ...
    $existing = Get-Job -Name $Name -ErrorAction SilentlyContinue | Where-Object { $_.State -eq 'Running' } | Select-Object -First 1
    if ($existing) {
        Write-Host "[WARN] $Name is already running" -ForegroundColor Yellow
        return
    }

    if (Test-Path $Path) {
        Write-Host "Starting $Name..." -ForegroundColor Yellow
        Start-Job -Name $Name -FilePath $Path -ArgumentList $Args | Out-Null
...
function Start-ManagedLoopService {
    param(
    ...
    $existing = Get-Job -Name $Name -ErrorAction SilentlyContinue | Where-Object { $_.State -eq 'Running' } | Select-Object -First 1
    if ($existing) {
        Write-Host "[WARN] $DisplayName is already running" -ForegroundColor Yellow
        return $true
    }

    if (-not (Test-Path $Path)) {
        Write-Host "[ERR] $DisplayName not found at $Path" -ForegroundColor Red
        return $false
    }

    Start-Job -Name $Name -ArgumentList $Path, $SleepSeconds -ScriptBlock {
        param($ServicePath, $LoopSleepSeconds)
        while ($true) {
            & $ServicePath
...
function Stop-ManagedService {
    ...
    if ($jobs.Count -eq 0) {
        Write-Host "[WARN] $DisplayName is not running" -ForegroundColor DarkYellow
        return $true
    }
    ...
    Write-Host "[OK] $DisplayName stopped" -ForegroundColor Yellow
    return $true
}

function Start-MasteryAccessoryLoopService {
    Start-ManagedLoopService -Name "MasteryAccessoryLoop" -DisplayName "Mastery Accessory Loop" -Path "C:\miratv_ingest\master_runner_loop_acc.bat" -SleepSeconds 60 | Out-Null
}

function Start-MainSeriesLoopService {
    Start-ManagedLoopService -Name "MainSeriesLoop" -DisplayName "Main Series Loop" -Path "C:\miratv_ingest\master_runner_loop.bat" -SleepSeconds 120 | Out-Null
}

function Start-MasterUploadLoopService {
    Start-ManagedLoopService -Name "MasterUploadLoop" -DisplayName "Master Upload Loop" -Path "C:\miratv_ingest\MASTER_UPLOAD_LOOP.bat" -SleepSeconds 60 | Out-Null
}
...
function Show-MasterControlDashboard {
    ...
    while ($true) {
        ...
        Write-DashboardServiceGroup -Title "`nADDITIONAL MANAGED SERVICES" -ServiceNames @('MasteryAccessoryLoop','MainSeriesLoop','MasterUploadLoop') -ServiceMap $serviceMap

        Write-Host ""
        Write-Host "MONITORED EXTERNAL / ADJACENT SERVICES" -ForegroundColor Cyan
        if ($config.BGC.Enabled) {
            $externalBgcStatus = Get-BgcWatcherStatus
            if ($externalBgcStatus.Running) {
                Write-DashboardLine -Label "BGC Watcher Service" -Value $externalBgcStatus.State -Status "OK"
            }
            else {
                Write-DashboardLine -Label "BGC Watcher Service" -Value $externalBgcStatus.State -Status "WARN"
            }
        }
        try {
            $dashProc = @(Get-CVIWebDashboardProcesses)
            if ($dashProc.Count -gt 0) {
                Write-DashboardLine -Label "CVI Web Dashboard" -Value ("RUNNING ({0} proc)" -f $dashProc.Count) -Status "OK"
            }
            else {
                Write-DashboardLine -Label "CVI Web Dashboard" -Value "STOPPED" -Status "WARN"
            }
        }
        catch {
            Write-DashboardLine -Label "CVI Web Dashboard" -Value "UNKNOWN" -Status "WARN"
        }
        try {
            $ollamaMonitor = Invoke-RestMethod -Uri "$($config.OllamaUrl)/api/tags" -TimeoutSec 5 -ErrorAction Stop
            if ($ollamaMonitor.models) {
                Write-DashboardLine -Label "Ollama Local AI" -Value "RUNNING" -Status "AI"
            }
            else {
                Write-DashboardLine -Label "Ollama Local AI" -Value "RUNNING / NO MODELS" -Status "WARN"
            }
        }
        catch {
            Write-DashboardLine -Label "Ollama Local AI" -Value "NOT RUNNING" -Status "WARN"
        }
...
do {
    ...
    switch ($choice) {
        "1" { 
            ...
            Start-MasteryAccessoryLoopService
            Start-MainSeriesLoopService
            Start-MasterUploadLoopService
        ...
        "38" {
            Stop-ManagedService -Name "MasteryAccessoryLoop" -DisplayName "Mastery Accessory Loop" | Out-Null
            Read-Host "Press Enter to continue"
        }
        "39" {
            Stop-ManagedService -Name "MainSeriesLoop" -DisplayName "Main Series Loop" | Out-Null
            Read-Host "Press Enter to continue"
        }
        "40" {
            Stop-ManagedService -Name "MasterUploadLoop" -DisplayName "Master Upload Loop" | Out-Null
            Read-Host "Press Enter to continue"
        }
        "99" { 
