...
function Ask-AI {
    ...
    if (-not (Test-BgcRule -RuleCode "COST_001")) {
        ...
        return @{
            ...
            source = "bgc"
            confidence = 1.0
            query_type = "governance_block"
            query_signature = "bgc::blocked::COST_001"
            retrieval_strategy = "bgc_enforcement"
            searched_memory_types = @()
        }
    }
...
"@ | Out-Null
    ...
    if (-not $script:BGC_PartialAllowed) {
        Write-Host "[BGC] Partial processing disabled — blocking plain Ollama fallback" -ForegroundColor Yellow
        return @{
            answer = "BGC behavior blocked degraded fallback because partial processing is currently disabled."
            source = "bgc"
            confidence = 1.0
            query_type = $queryType
            query_signature = $querySignature
            retrieval_strategy = "bgc_partial_block"
            searched_memory_types = $searchedMemoryTypes
        }
    }
