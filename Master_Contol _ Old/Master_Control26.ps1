...
function Get-PCDEMatchedQueryTypes {
    ...
    foreach ($prop in $Rules.query_types.PSObject.Properties) {
        $queryTypeName = $prop.Name
        $queryTypeDef = $prop.Value

        if (Test-PCDEQueryTypeMatch -Question $Question -TriggerConfig $queryTypeDef.triggers) {
...
function Get-PCDESelectedQueryType {
    ...
    if ($matches.Count -eq 0) {
        ...
        return [PSCustomObject]@{
            ...
            Config = $Rules.query_types.$defaultType
            Matches = @()
        }
    }

    if ($Rules.routing_rules.use_first_matching_query_type -eq $true) {
...
function Invoke-PCDEMemorySearchByType {
    ...
    Write-Host ("   🔍 Searching {0}..." -f $label) -ForegroundColor DarkCyan
...
function Set-PCDECandidateScoresFromRules {
    ...
    foreach ($candidate in @($Candidates)) {
        ...
        if ($Rules.memory_types.$memoryType.default_priority) {
