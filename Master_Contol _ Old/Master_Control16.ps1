...
function Invoke-OllamaMemoryArbiter {
    ...
    foreach ($candidate in $sortedCandidates) {
        $candidateRows = Format-MemoryRows -Rows $candidate.Rows -Kind $candidate.Source
        $candidateSections += @"
[CANDIDATE]
...
"@
    ...
    $candidateBlock = if ($candidateSections.Count -gt 0) {
        $candidateSections -join "`n"
    } else {
        "[NO_CANDIDATES] No retrieved memory candidates were found. [/NO_CANDIDATES]"
    }
...
Your job:
1. Review the user question.
2. Review every retrieved memory candidate.
3. Choose the best candidate or synthesize a grounded answer from multiple candidates.
4. Stay grounded in retrieved memory when candidates are present.
5. If the evidence is weak or conflicting, say so plainly.
6. If there are no candidates, answer normally but say that no stored memory matched.
...
Return STRICT JSON ONLY with this schema:
{
  ...
  "rationale": "brief explanation of why this answer won"
}
"@
...
"@
    ...
    Write-Host "🤖 Sending retrieved memory to Ollama for final arbitration..." -ForegroundColor Yellow
