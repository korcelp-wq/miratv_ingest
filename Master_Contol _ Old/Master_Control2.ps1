#!/usr/bin/env pwsh
# Master Control with FULL Database Integration
...
function Show-Header {
    ...
    Write-Host "║     18 Tables · 272 AI Memories · Local AI Ready       ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
}
...
function Show-Status {
    ...
    if ($test -match "rows") {
        ...
        $decCount = & $dashboardQuery -Sql "SELECT COUNT(*) as c FROM pcde_declarative_memory" 2>$null
        if ($decCount -match '"c":\s*(\d+)') {
            Write-Host "   📚 Declarative: $($matches[1])" -ForegroundColor Cyan
        }
...
while(`$true) {
    ...
    Write-Host "[`$timestamp] 🔍 Running Knowledge Miner..."
    & 'C:\miratv_ingest\workers\KnowledgeMiner.ps1'
    
    Write-Host "[`$timestamp] 🔗 Running Relationship Finder..."
...
while(`$true) {
    `$timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    Write-Host "[`$timestamp] 📤 Running Accessory Upload..."
...
while(`$true) {
    `$timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    Write-Host "[`$timestamp] 🏃 Running Master Runner..."
...
VALUES ('$script:aiSessionId', 'q_$(Get-Random)', '$Question'),
       ('$script:aiSessionId', 'a_$(Get-Random)', '$answer')
"@
...
"@
    ...
    if ($history.rows) {
        ...
        foreach ($item in $history.rows) {
            ...
            Write-Host "$prefix $($item.slot_value)"
        }
    }
}
...
function Show-Menu {
    ...
    Write-Host "    4) Start Learning Loop"
    Write-Host "    5) Start Accessory Loop"
    Write-Host "    6) Start Runner Loop"
    Show-AIMenu
...
do {
    ...
    switch ($choice) {
        ...
        "3" { Get-Job | Format-Table Name, State; Read-Host "Press Enter" }
        ...
        "20" { Start-AISession; Read-Host "Press Enter" }
        "21" { 
            $q = Read-Host "Your question"
            $r = Ask-AI -Question $q
            Write-Host "`n🤖 [$($r.source)] $($r.answer)" -ForegroundColor Green
            Read-Host "`nPress Enter"
        }
        "22" { Show-ChatHistory; Read-Host "Press Enter" }
        "23" { $script:aiSessionId = $null; Write-Host "Session cleared"; Read-Host "Press Enter" }
        "24" { Show-AIStats; Read-Host "Press Enter" }
        ...
        "33" { Get-ChildItem $logDir -ErrorAction SilentlyContinue | Select -Last 3 | ForEach-Object { Get-Content $_.FullName -Tail 3 }; Read-Host "Press Enter" }
