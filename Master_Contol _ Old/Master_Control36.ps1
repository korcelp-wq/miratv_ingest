...
function Invoke-PCDELongTermRecall {
    ...
    try {
        $rows = @(Invoke-SqlQueryObjects -Sql $sql -DatabaseName "pcde_memory" -Context 'LongTermRecallProc')
        if ($null -eq $rows) {
            Write-DebugLog -Category 'RECALL' -Message 'Stored procedure returned null response.'
            return @()
        }
        $rows = @($rows | Where-Object { $null -ne $_ })
        Write-DebugLog -Category 'RECALL' -Message ("Normalized long-term recall row count = {0}" -f $rows.Count)

        if ($rows.Count -gt 0) {
            Write-Host "   [OK] Long-term recall returned $($rows.Count) row(s)" -ForegroundColor Green
            $first = $rows[0] | ConvertTo-Json -Depth 6 -Compress
            Write-DebugLog -Category 'RECALL' -Message ("First long-term row: {0}" -f $first)
        }
        else {
            Write-DebugLog -Category 'RECALL' -Message 'Stored procedure returned zero rows after normalization.'
        }
