...
function Safe-SqlQuery {
    ...
    try {
        $rows = & $queryScript -Sql $Sql -Db $Db 4>$null
...
if ($procedural) {
    Write-Host "   ✅ Found in Procedural Memory ($($procedural.Count) rows)" -ForegroundColor Green

    foreach ($row in $procedural | Select-Object -First 5) {
        Write-Host "      - $($row.procedure_name) [$($row.domain)]" -ForegroundColor DarkCyan
    }
}
...
"@ 2>$null
    ...
    $lines = foreach ($row in $Rows | Select-Object -First 5) {
        ...
        if ($props -contains 'procedure_name') {
            "- $($row.procedure_name) | domain=$($row.domain) | type=$($row.procedure_type) | desc=$($row.description)"
        }
