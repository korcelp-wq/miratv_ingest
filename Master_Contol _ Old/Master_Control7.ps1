...
function Get-Keywords {
    ...
    foreach ($word in $words) {
        if ($word.Length -gt 2 -and $stopWords -notcontains $word) {
            $keywords += $word
        }
    }
...
function Ask-AI {
    ...
    $procFields = @('procedure_name', 'description', 'parameters', 'tags')
    $procWhere = Build-WhereClause -Fields $procFields -Keywords $keywords
    
    $proceduralSql = @"
SELECT procedure_id, procedure_name, description, confidence_score, usage_count
...
"@
    ...
    if (-not $memoryResult) {
        ...
        $decFields = @('fact', 'keywords', 'category', 'source')
        $decWhere = Build-WhereClause -Fields $decFields -Keywords $keywords
        
        $declarativeSql = @"
SELECT memory_id, fact, category, confidence, source
...
"@
    ...
    if (-not $memoryResult) {
        ...
        $assocFields = @('relation_type', 'pattern', 'notes')
        $assocWhere = Build-WhereClause -Fields $assocFields -Keywords $keywords
        
        $associativeSql = @"
SELECT relation_id, procedure_id, related_procedure_id, relation_type, strength
...
"@
    ...
    if (-not $memoryResult) {
        ...
        $aiFields = @('key_data', 'context')
        $aiWhere = Build-WhereClause -Fields $aiFields -Keywords $keywords
        
        $aiMemorySql = @"
SELECT memory_id, key_data, confidence, context
FROM pcde_ai_memory 
WHERE $aiWhere
ORDER BY confidence DESC, access_count DESC
LIMIT 3
...
"@ 2>$null
    ...
    try {
        ...
        if ($availableModels.Count -eq 0) {
            return @{
                answer = "I'm Mira, your AI assistant. I can help answer questions about the system."
                source = "fallback"
                confidence = 0.5
            }
        }
    ...
    # Ask Ollama
    $prompt = @"
You are MiraTV AI assistant. Answer this question concisely: $Question
"@
...
"@ 2>$null
        ...
        return @{
            answer = "I'm Mira, your AI assistant. How can I help you today?"
            source = "fallback"
            confidence = 0.5
        }
    }
