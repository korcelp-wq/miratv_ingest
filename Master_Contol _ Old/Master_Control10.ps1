...
function Show-Menu {
    ...
    Write-Host "   34) Check Active Processes (sp_whoisactive)"
...
'@ }
                    ...
                    if ($scriptBlock) {
                        ...
                        $result = Invoke-PowerShellCommand -Command "Start-Job -Name '$actualService' -ScriptBlock { $escapedScript }" -Description "Starting $actualService"
                        $answer = if ($result.success) { 
