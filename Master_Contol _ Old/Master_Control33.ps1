...
function Invoke-PCDELongTermRecall {
    ...
    if ([string]::IsNullOrWhiteSpace($SearchTerm)) {
        Write-Host "   ⚪ Empty search term, skipping long-term recall" -ForegroundColor DarkGray
        return @()
    }

    $escapedTerm = $SearchTerm -replace "'", "''"
    $escapedType = $QueryType -replace "'", "''"

    Write-Host "   🗃️ Calling long-term recall stored procedure..." -ForegroundColor Cyan
    Write-Host "      Term: '$SearchTerm'" -ForegroundColor DarkGray
    Write-Host "      Type: $QueryType" -ForegroundColor DarkGray
    Write-Host "      Limit: $MaxRowsPerTarget" -ForegroundColor DarkGray

    $sql = "CALL xpdgxfsp_pcde_memory.pcde_long_term_memory_recall('$escapedTerm', '$escapedType', $MaxRowsPerTarget);"

    try {
        $rows = @(Invoke-SqlQueryObjects -Sql $sql -DatabaseName "pcde_memory")

        if ($rows -and $rows.Count -gt 0) {
            Write-Host "   ✅ Long-term recall returned $($rows.Count) row(s)" -ForegroundColor Green
        }
        else {
        ...
        Write-Host "   ❌ Long-term recall failed: $($_.Exception.Message)" -ForegroundColor Red
        return @()
    }
}

function Test-LongTermMemoryTables {
    Write-Host "`n🧪 Testing Long-Term Memory Recall Procedure" -ForegroundColor Cyan
    Write-Host "============================================" -ForegroundColor Cyan

    $sql = "CALL xpdgxfsp_pcde_memory.pcde_long_term_memory_recall('CVI', 'acronym_definition', 5);"

    try {
        $rows = @(Invoke-SqlQueryObjects -Sql $sql -DatabaseName "pcde_memory")
        Write-Host "✅ Stored procedure reachable" -ForegroundColor Green
        Write-Host "   Rows returned: $($rows.Count)" -ForegroundColor Green
        if ($rows.Count -gt 0) {
            Show-MemoryPreview -Rows $rows -Title "Long-Term Recall Preview" -Kind "long_term"
        }
    }
    catch {
        Write-Host "❌ Stored procedure test failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
