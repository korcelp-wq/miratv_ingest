...
Labels like Procedural Memory, Declarative Memory, Associative Memory, Working Memory, AI Learning Memory, Long Term Memory, and synthetic procedures are only source metadata. They are not usually the answer.

CRITICAL OUTPUT RULES:
- You MUST answer the question directly in plain language.
- You MUST NOT return raw rows, key-value pairs, field mappings, predicates, relation labels, notes fragments, slot values, or database-style outputs as the answer.
- You MUST NOT echo candidate records or memory labels as the answer.
- You MUST synthesize meaning from the evidence.
- If the question is definitional, provide a definition first, then a short explanation.
- Prefer strong system-specific evidence over incidental keyword matches.
- Prefer higher-ranked and semantically aligned evidence over fragments.
- Use contextual evidence only to support or clarify.
- Ignore repetitive or low-signal rows when stronger evidence exists.
...
4. Prefer row fields such as procedure_name, description, predicate, object_value, relation_target, notes, slot_value, key_data only as clues, not as the final answer format.
5. If the retrieved evidence answers the question, answer directly in plain language.
6. If multiple candidates help, synthesize them into one coherent answer.
...
11. Do not return bullet lists of raw evidence unless the user explicitly asks to inspect the evidence.
12. Return JSON only.
...
Return STRICT JSON ONLY with this schema:
{
  ...
  "grounded": true,
  "confidence": 0.0,
  "answer": "answer the user's original question directly in natural language, not as raw rows",
...
if ($longTermRows -and @($longTermRows).Count -gt 0) {
    ...
    Write-Host "[LONG] No results in Long Term Memory" -ForegroundColor DarkGray
}
