...
function Import-PCDEJudgementRules {
    param(
        [string]$YamlPath = "C:\miratv_ingest\pcde_judgement.yaml"
    )

    if (-not (Test-Path $YamlPath)) {
        throw "Judgement rules YAML not found: $YamlPath"
    }
    ...
    if (-not $parsed -or -not $parsed.pcde_judgement) {
        throw "Judgement YAML is invalid or not in expected format (pcde_judgement root key missing): $YamlPath"
    }
...
function Resolve-PCDEJudgementQueryType {
    ...
    foreach ($patternGroup in $patterns) {
        ...
        foreach ($pattern in $candidatePatterns) {
            ...
            if ($Question -match [string]$pattern) {
                if ($resolved -eq 'general' -or $resolved -eq 'definition') {
...
function Ask-AI {
    ...
    try {
        ...
        Write-Host "   [WARN] Could not load judgement rules YAML. Proceeding without judgement overrides." -ForegroundColor Yellow
        $judgementRules = $null
    }
