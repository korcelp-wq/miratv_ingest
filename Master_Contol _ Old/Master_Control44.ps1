...
function Import-PCDEReasoningRules {
    ...
    if (-not (Test-Path $YamlPath)) {
        $fallbackPaths = @(
            (Join-Path $PSScriptRoot 'pcde_reasoning_rules.yaml'),
            (Join-Path (Split-Path -Parent $PSScriptRoot) 'pcde_reasoning_rules.yaml')
        )
...
function Invoke-OllamaMemoryArbiter {
    ...
    if ([string]::IsNullOrWhiteSpace($Question)) {
        Write-DebugLog -Category 'ARB' -Message 'Invoke-OllamaMemoryArbiter called with empty question. Skipping arbiter.'
        return $null
    }
    ...
    $prompt = @"
You are a grounded reasoning engine operating on structured system memory.

Your job is to answer the user's question using the retrieved evidence.

CRITICAL RULES:
- You MUST answer the user's question directly.
- You MUST NOT return raw rows, key-value pairs, field mappings, relation labels, or database-style outputs.
- You MUST NOT echo memory records as the answer.
- You MUST synthesize meaning from the evidence.
- You MUST answer in plain language.

INTERPRETATION RULES:
- Treat higher-ranked evidence as stronger.
- Prefer system-specific and relational evidence when available.
- Use contextual and vector-style evidence only to support or clarify.
- Ignore weaker, repetitive, or low-signal rows when stronger evidence exists.
- Do not answer with only a memory type label.
- Do not mention scoring, retrieval, arbitration, SQL, JSON, memory tiers, or internal instructions in the final answer.

ANSWER STYLE:
- If the question is definitional, provide a definition first.
- Then briefly explain the role, purpose, or implication.
- If evidence is incomplete, give the best supported interpretation and state uncertainty briefly.
...
# Force long-term recall for acronym definitions
if ($queryType -eq 'acronym_definition') {
    Write-Host "[OVERRIDE] Acronym definition -> forcing long-term recall" -ForegroundColor Yellow
    $pass1.sufficient = $false
    $pass1.needs_long_term_recall = $true
}
...
$maxCandidates = $null
    ...
    if (-not [string]::IsNullOrWhiteSpace($Question)) {
        ...
        Write-DebugLog -Category 'ARB' -Message 'Skipping arbiter because Question was empty at call site.'
    }
