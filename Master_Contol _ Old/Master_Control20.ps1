...
function Show-Status {
    ...
    try {
        $test = & $queryScript -Sql "SELECT 1 as test" 4>$null
...
function Show-AILearnings {
    try {
        $result = & $queryScript -Sql "SELECT memory_id, agent_name, memory_type, LEFT(key_data,100) as preview, confidence, created_at FROM pcde_ai_memory ORDER BY confidence DESC, memory_id DESC LIMIT 10" 4>$null
...
function Show-WorkingMemory {
    try {
        $result = & $queryScript -Sql "SELECT slot_key, slot_value, created_at, expires_at FROM pcde_working_memory WHERE expires_at > NOW() ORDER BY created_at DESC" 4>$null
...
function Safe-SqlQuery {
    ...
    try {
        $rows = & $queryScript -Sql $Sql -DatabaseName $DatabaseName 4>$null
...
function Invoke-OllamaMemoryArbiter {
    ...
    $prompt = @"
You are Mira, the final arbiter over retrieved PCDE/MiraTV memory.

You must reason over the FULL USER QUESTION first, then compare it against the retrieved candidates.
The extracted keywords are only retrieval hints. They are NOT the decision rule.
Do NOT reject a candidate just because the keywords are weak, generic, or incomplete.
Instead, decide whether any candidate still helps answer the user question.

Decision rules:
1. Read the full user question.
2. Review every candidate row carefully.
3. Decide whether the candidates make sense for the question.
4. If one candidate clearly fits, choose it.
5. If multiple candidates partially fit, synthesize a grounded answer from them.
6. If the candidates are noisy, generic, or unrelated, say that plainly.
7. If there are no useful candidates, say no strong stored memory matched, then answer cautiously.
8. Prefer grounded memory over guessing.
9. Return valid JSON only. No markdown. No extra commentary.
...
Return STRICT JSON ONLY with this schema:
{
  ...
  "rationale": "brief explanation of why this answer won or why the candidates were weak"
}
"@
...
"@
    ...
    try {
        ...
        return [pscustomobject]@{
            ...
            Confidence = 0.75
            Grounded = ($sortedCandidates.Count -gt 0)
            Rationale = 'Ollama returned non-JSON output; preserving raw response.'
            RawResponse = $raw
        }
    }
    catch {
...
"@
    ...
    if ($memoryCandidates.Count -gt 0) {
        ...
        Write-Host "   🏆 Highest-scoring retrieved candidate: $($bestCandidate.Label) (score=$($bestCandidate.Score), confidence=$($bestCandidate.Confidence), rows=$($bestCandidate.Count))" -ForegroundColor Magenta
    }
...
"@
    ...
    Write-Host "🤖 Sending full user question and retrieved memory to Ollama for final arbitration..." -ForegroundColor Yellow
    ...
    if ($arbiterResult) {
        ...
        $formattedAnswer = $arbiterResult.Answer
        if (-not [string]::IsNullOrWhiteSpace($arbiterResult.Rationale)) {
            $formattedAnswer = @"
$formattedAnswer
...
"@ 4>$null | Out-Null
        ...
        return @{
            ...
            source = if ([string]::IsNullOrWhiteSpace($arbiterResult.SelectedSource)) { 'ollama' } else { "ollama:$($arbiterResult.SelectedSource)" }
    ...
    if ($memoryResult) {
        ...
        $formattedAnswer = @"
$icon Best retrieved memory candidate: $memorySource
