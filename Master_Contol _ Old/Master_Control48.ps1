...
function Import-PCDEYamlFile {
    ...
    if (-not (Test-Path -LiteralPath $YamlPath)) {
        throw "YAML not found: $YamlPath"
    }

    $yamlText = Get-Content -LiteralPath $YamlPath -Raw -Encoding UTF8
    if ($null -eq $yamlText) {
        throw "YAML read returned null: $YamlPath"
    }

    $yamlText = $yamlText.Trim()
    if ([string]::IsNullOrWhiteSpace($yamlText)) {
        throw "YAML file is empty: $YamlPath"
    }

    if ($yamlText.Length -gt 0 -and [int][char]$yamlText[0] -eq 0xFEFF) {
        $yamlText = $yamlText.Substring(1)
    }
...
function Import-PCDEReasoningRules {
    ...
    $candidates = @()
    if (-not [string]::IsNullOrWhiteSpace($YamlPath)) { $candidates += $YamlPath }
    $candidates += @(
        "C:\miratv_ingest\pcde_reasoning_rules.yaml",
        "C:\miratv_ingest\dashboard\pcde_reasoning_rules.yaml",
        (Join-Path -Path $scriptPath -ChildPath "pcde_reasoning_rules.yaml"),
        (Join-Path -Path $scriptPath -ChildPath "dashboard\pcde_reasoning_rules.yaml"),
    ...
    foreach ($candidate in $candidates) {
        try {
            $parsed = Import-PCDEYamlFile -YamlPath $candidate
            if ($null -ne $parsed) {
                Write-DebugLog -Category 'YAML' -Message ("Loaded reasoning YAML from: {0}" -f $candidate)
                return $parsed
            }
        }
        catch {
            $lastError = $_.Exception.Message
            Write-DebugLog -Category 'YAML' -Message ("Reasoning YAML load failed for {0}: {1}" -f $candidate, $lastError)
        }
    }

    if ($lastError) {
        throw "Reasoning rules YAML could not be loaded. Last error: $lastError"
    }

    throw "Reasoning rules YAML not found in any expected location."
}

function Import-PCDEJudgementRules {
    param(
        [string]$YamlPath = ""
    )
    ...
    if (-not [string]::IsNullOrWhiteSpace($YamlPath)) { $candidates += $YamlPath }
    $candidates += @(
        "C:\miratv_ingest\pcde_judgement.yaml",
        "C:\miratv_ingest\pcde_judgement_fixed.yaml",
        "C:\miratv_ingest\dashboard\pcde_judgement.yaml",
        (Join-Path -Path $scriptPath -ChildPath "pcde_judgement.yaml"),
        (Join-Path -Path $scriptPath -ChildPath "pcde_judgement_fixed.yaml"),
        "/mnt/data/pcde_judgement.yaml",
        "/mnt/data/pcde_judgement_fixed.yaml"
    ) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique
    ...
    foreach ($candidate in $candidates) {
        try {
            $parsed = Import-PCDEYamlFile -YamlPath $candidate
            if (-not $parsed -or -not $parsed.pcde_judgement) {
                throw "Judgement YAML root key 'pcde_judgement' missing: $candidate"
            }

            Write-DebugLog -Category 'YAML' -Message ("Loaded judgement YAML from: {0}" -f $candidate)
            return $parsed
        }
        catch {
            $lastError = $_.Exception.Message
            Write-DebugLog -Category 'YAML' -Message ("Judgement YAML load failed for {0}: {1}" -f $candidate, $lastError)
        }
    }

    if ($lastError) {
        throw "Judgement rules YAML could not be loaded. Last error: $lastError"
    }

    throw "Judgement rules YAML not found in any expected location."
}
...
function Test-PCDEQueryTypeMatch {
    ...
    # If TriggerConfig is null, no match
    ...
    $hasDefault = $false
    try {
        if ($TriggerConfig.PSObject.Properties.Name -contains 'default') {
    ...
    $maxTerms = 0
    try {
        if ($TriggerConfig.PSObject.Properties.Name -contains 'max_terms' -and $null -ne $TriggerConfig.max_terms) {
            $hasMaxTerms = $true
    ...
    # Check starts_with
    try {
        if ($TriggerConfig.PSObject.Properties.Name -contains 'starts_with' -and $TriggerConfig.starts_with) {
    ...
    # Check contains
    try {
        if ($TriggerConfig.PSObject.Properties.Name -contains 'contains' -and $TriggerConfig.contains) {
    ...
    # Check regex
    try {
        if ($TriggerConfig.PSObject.Properties.Name -contains 'regex' -and $TriggerConfig.regex) {
...
function Get-PCDECandidateEvidenceClass {
    param(
        [string]$Source,
        [object[]]$Rows = @()
    )

    $sourceText = ([string]$Source).ToLowerInvariant()

    switch ($sourceText) {
        'declarative' { return 'relational' }
        'procedural' { return 'relational' }
        'working' { return 'relational' }
        'synthetic_procedures' { return 'relational' }
        'associative' { return 'vector' }
        'ai_memory' { return 'vector' }
        'long_term_relational' { return 'relational' }
        'long_term_vector' { return 'vector' }
        'long_term' {
            $rowClasses = @($Rows | ForEach-Object {
                if ($null -ne $_ -and $_.PSObject.Properties.Name -contains 'evidence_class' -and -not [string]::IsNullOrWhiteSpace([string]$_.evidence_class)) {
                    ([string]$_.evidence_class).ToLowerInvariant()
                }
            } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique)

            if ($rowClasses.Count -eq 1) { return $rowClasses[0] }
            if ($rowClasses.Count -gt 1) { return 'hybrid' }
            return 'relational'
        }
...
function Split-PCDELongTermRowsByEvidenceClass {
    param(
        [object[]]$Rows = @()
    )
    ...
    foreach ($row in @($Rows)) {
        $rowClass = ''
        if ($null -ne $row -and $row.PSObject.Properties.Name -contains 'evidence_class' -and -not [string]::IsNullOrWhiteSpace([string]$row.evidence_class)) {
            $rowClass = ([string]$row.evidence_class).ToLowerInvariant()
        }

        switch ($rowClass) {
            'relational' { $relational += $row }
            'vector' { $vector += $row }
            default { $unknown += $row }
        }
    }
...
function Resolve-PCDEJudgementQueryType {
    param(
        [string]$Question,
        [string]$CurrentQueryType = 'general',
        [object]$JudgementRules = $null
    )

    $resolved = $CurrentQueryType
    $normalizedQuestion = Get-PCDENormalizedQuestion -Question $Question

    if ($normalizedQuestion -match '^(how does|how do you|how do we|how do i|how to|how can i)\b') {
        return 'procedural'
    }

    if ($null -eq $JudgementRules -or $null -eq $JudgementRules.pcde_judgement -or $null -eq $JudgementRules.pcde_judgement.tasks) {
        return $resolved
    }

    $task = $JudgementRules.pcde_judgement.tasks.classify_question
    if ($null -eq $task -or $task.enabled -ne $true -or $null -eq $task.patterns) {
        return $resolved
    }

    $patterns = $task.patterns.PSObject.Properties
    foreach ($patternGroup in $patterns) {
        $candidateType = [string]$patternGroup.Name
        $candidatePatterns = @($patternGroup.Value)

        foreach ($pattern in $candidatePatterns) {
            if ([string]::IsNullOrWhiteSpace([string]$pattern)) { continue }
            if ($normalizedQuestion -match [string]$pattern) {
                if ($resolved -eq 'general' -or $resolved -eq 'definition' -or [string]::IsNullOrWhiteSpace($resolved)) {
                    return $candidateType
                }

                if ($candidateType -eq 'procedural' -and $resolved -ne 'acronym_definition') {
                    return $candidateType
                }
            }
        }
    ...
    if ([string]::IsNullOrWhiteSpace($resolved) -and $task.default) {
        return [string]$task.default
    }

    return $resolved
}

function Get-PCDEJudgementDecision {
    param(
        [string]$Question,
        [string]$QueryType,
        [object[]]$Candidates = @(),
    ...
    if ($null -eq $JudgementRules -or $null -eq $JudgementRules.pcde_judgement) {
        return $result
    }

    $tasks = $JudgementRules.pcde_judgement.tasks
    $hasCandidates = (@($Candidates).Count -gt 0)
    $hasRelational = @($Candidates | Where-Object { (Get-PCDECandidateEvidenceClass -Source $_.Source -Rows $_.Rows) -eq 'relational' }).Count -gt 0
    $hasVector = @($Candidates | Where-Object { (Get-PCDECandidateEvidenceClass -Source $_.Source -Rows $_.Rows) -eq 'vector' }).Count -gt 0
    $topCandidate = @($Candidates | Sort-Object -Property Score, Confidence, Count -Descending | Select-Object -First 1)
    $topEvidenceClass = if ($topCandidate.Count -gt 0) { Get-PCDECandidateEvidenceClass -Source $topCandidate[0].Source -Rows $topCandidate[0].Rows } else { '' }

    if ($tasks.determine_evidence_dominance.enabled -eq $true) {
        $ruleSet = $tasks.determine_evidence_dominance.rules
        $dominanceRule = $null
        if ($ruleSet.PSObject.Properties.Name -contains $QueryType) {
            $dominanceRule = $ruleSet.$QueryType
        }
        elseif ($ruleSet.PSObject.Properties.Name -contains 'default') {
            $dominanceRule = $ruleSet.default
        }

        if ($dominanceRule -and $dominanceRule.dominant) {
            $result.DominantEvidence = [string]$dominanceRule.dominant
        }
    }

    if ($tasks.judge_evidence_sufficiency.enabled -eq $true) {
        if (-not $hasCandidates) {
            $result.EvidenceSufficiency = 'insufficient'
        }
        elseif (-not $hasRelational -and $hasVector) {
            $result.EvidenceSufficiency = 'insufficient'
        }
        elseif ($topEvidenceClass -eq 'vector' -and $hasRelational) {
            $result.EvidenceSufficiency = 'sufficient_but_qualify'
        }
        else {
            $result.EvidenceSufficiency = 'sufficient'
        }
    }

    if ($tasks.decide_recall.enabled -eq $true) {
        $forceTypes = @($tasks.decide_recall.rules.force_recall_if_query_type)
        $keywordTerms = @($Keywords | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
        if ($QueryType -in $forceTypes) {
            $result.NeedsRecallOverride = $true
        }
        elseif (-not $hasCandidates) {
            $result.NeedsRecallOverride = $true
        }
        elseif ($result.EvidenceSufficiency -eq 'insufficient') {
            $result.NeedsRecallOverride = $true
        }

        if ($keywordTerms.Count -gt 0) {
            $result.RecallTerms = $keywordTerms
        }
        elseif ($Pass1 -and $Pass1.recall_terms) {
            $result.RecallTerms = @($Pass1.recall_terms)
        }
        else {
            $result.RecallTerms = @($Question)
        }
    }

    if ($tasks.decide_qualification.enabled -eq $true) {
        if (-not $hasRelational -and $hasVector) {
            $result.Qualification = 'answer_with_uncertainty'
        }
        elseif ($result.EvidenceSufficiency -eq 'sufficient_but_qualify') {
            $result.Qualification = 'answer_with_scope_note'
        }
        elseif ($result.EvidenceSufficiency -eq 'insufficient') {
            $result.Qualification = 'defer_answer'
        }
    }
...
function Apply-PCDEJudgementDominanceToCandidates {
    param(
        [object[]]$Candidates = @(),
    ...
    $updated = @()
    $decision = Get-PCDEJudgementDecision -Question '' -QueryType $QueryType -Candidates $Candidates -Keywords @() -Pass1 $null -JudgementRules $JudgementRules
    $dominant = $decision.DominantEvidence
    $hasRelational = @($Candidates | Where-Object { (Get-PCDECandidateEvidenceClass -Source $_.Source -Rows $_.Rows) -eq 'relational' }).Count -gt 0

    foreach ($candidate in @($Candidates)) {
        $candidateClass = Get-PCDECandidateEvidenceClass -Source $candidate.Source -Rows $candidate.Rows
        $adjustedScore = [double]$candidate.Score

        if ($candidateClass -eq $dominant) {
            $adjustedScore += 0.08
        }
        elseif ($candidateClass -eq 'vector' -and $hasRelational) {
            $adjustedScore -= 0.06
        }

        if ($QueryType -eq 'acronym_definition' -and $candidate.Source -eq 'declarative') {
            $adjustedScore += 0.04
        }

        $candidate | Add-Member -NotePropertyName EvidenceClass -NotePropertyValue $candidateClass -Force
        $candidate.Score = [Math]::Round($adjustedScore, 4)
        $updated += $candidate
    }

    return @($updated | Sort-Object -Property @{Expression='Score';Descending=$true}, @{Expression='Confidence';Descending=$true}, @{Expression='Count';Descending=$true})
}
...
function ConvertTo-JsonForSql {
    ...
    try {
        ...
        return (ConvertTo-SqlLiteral -Value $json)
    }
    catch {
        return "NULL"
    }
...
function Get-ConfidenceNote {
    ...
    if ($Grounded -and $Confidence -ge 0.90) { return "high_grounded_confidence" }
    if ($Grounded -and $Confidence -ge 0.75) { return "grounded_confidence" }
    if ($Grounded) { return "grounded_low_confidence" }
    if ($Confidence -ge 0.75) { return "ungrounded_high_confidence" }
    return "low_confidence"
}

function Write-PCDEArbitrationHistory {
    param(
        [Parameter(Mandatory = $true)][string]$Question,
        [string]$QueryType = "general",
        [string]$ArbiterMode = "general_resolution",
        [string]$SqlSearchTerm = "",
        [string]$ModelName = "",
        [string]$AnswerText = "",
        [double]$Confidence = 0.0,
        [bool]$Grounded = $false,
        [object[]]$SelectedEvidence = @(),
        [object[]]$AllEvidence = @(),
        [string]$OutcomeStatus = "provisional",
        [string]$CorrectionNote = "",
        [string]$ConversationId = "",
        [string]$SessionId = ""
    )

    $confidenceNote = Get-ConfidenceNote -Confidence $Confidence -Grounded $Grounded

    $sql = @"
INSERT INTO pcde_arbitration_history (
    question,
    query_type,
    arbiter_mode,
    sql_search_term,
...
VALUES (
    $(ConvertTo-SqlLiteral $Question),
    $(ConvertTo-SqlLiteral $QueryType),
    $(ConvertTo-SqlLiteral $ArbiterMode),
    $(ConvertTo-SqlLiteral $SqlSearchTerm),
    $(ConvertTo-SqlLiteral $ModelName),
    $(ConvertTo-SqlLiteral $AnswerText),
    $(ConvertTo-SqlLiteral $confidenceNote),
    $(ConvertTo-JsonForSql $SelectedEvidence),
    $(ConvertTo-JsonForSql $AllEvidence),
    $(ConvertTo-SqlLiteral $OutcomeStatus),
    $(ConvertTo-SqlLiteral $CorrectionNote),
    $(ConvertTo-SqlLiteral $ConversationId),
    $(ConvertTo-SqlLiteral $SessionId)
);
"@

    $ok = Invoke-SilentSqlNonQuery -Sql $sql -DatabaseName "pcde_memory"
    if (-not $ok) {
        Write-DebugLog -Category 'WRITEBACK' -Message 'Failed to insert into pcde_arbitration_history.'
    }
    else {
        Write-DebugLog -Category 'WRITEBACK' -Message 'Inserted arbitration history row.'
    }

    return $ok
}

function Get-ProceduralDomainFromQuestion {
    param([string]$Question)

    $q = ([string]$Question).ToLowerInvariant()

    if ($q -match 'sql|database|mysql|mariadb|stored procedure|query') { return 'database' }
    if ($q -match 'powershell|ps1|script|master_control|query.ps1') { return 'automation' }
    if ($q -match 'ollama|prompt|model|arbiter') { return 'reasoning' }
    if ($q -match 'yaml|config|rules') { return 'configuration' }

    return 'general'
}

function Get-ProceduralNameFromQuestion {
    param([string]$Question)
...
function Promote-ToProceduralMemory {
    param(
        [Parameter(Mandatory = $true)][string]$Question,
        [Parameter(Mandatory = $true)][string]$AnswerText,
        [double]$Confidence = 0.0,
        [bool]$Grounded = $false,
        [string]$ProcedureType = "learned_workflow",
        [string]$Domain = "",
        [string]$WhyItExists = "Captured from successful arbitration cycle.",
        [bool]$RequireProceduralQueryType = $true,
        [string]$QueryType = "general"
    )

    if ($RequireProceduralQueryType -and $QueryType -ne 'procedural') {
        Write-DebugLog -Category 'WRITEBACK' -Message 'Skipping procedural promotion because query_type is not procedural.'
        return $false
    }

    if (-not $Grounded -or $Confidence -lt 0.75) {
        Write-DebugLog -Category 'WRITEBACK' -Message 'Skipping procedural promotion because answer is not grounded enough.'
        return $false
    }

    if ([string]::IsNullOrWhiteSpace($Domain)) {
        $Domain = Get-ProceduralDomainFromQuestion -Question $Question
    }

    $procedureName = Get-ProceduralNameFromQuestion -Question $Question
...
SELECT
    $(ConvertTo-SqlLiteral $procedureName),
    $(ConvertTo-SqlLiteral $ProcedureType),
    $(ConvertTo-SqlLiteral $Domain),
    $(ConvertTo-SqlLiteral $AnswerText),
    $(ConvertTo-SqlLiteral $WhyItExists),
    1
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1
    FROM pcde_procedure_registry
    WHERE procedure_name = $(ConvertTo-SqlLiteral $procedureName)
      AND domain = $(ConvertTo-SqlLiteral $Domain)
);
"@

    $ok = Invoke-SilentSqlNonQuery -Sql $sql -DatabaseName "pcde_memory"
    if ($ok) {
        Write-DebugLog -Category 'WRITEBACK' -Message ("Promoted procedural memory: {0}" -f $procedureName)
    }
    else {
        Write-DebugLog -Category 'WRITEBACK' -Message ("Failed procedural promotion: {0}" -f $procedureName)
    }
...
function Get-MemoryCandidateScore {
    ...
    $baseScore = switch ($Source) {
        ...
        'ai_memory'   { 0.80 }
        'long_term'   { 0.89 }
        'long_term_relational' { 0.94 }
        'long_term_vector' { 0.76 }
        default       { 0.70 }
    }
...
function New-MemoryCandidate {
    ...
    return [pscustomobject]@{
        ...
        Score = $score
        Count = @($Rows).Count
        EvidenceClass = Get-PCDECandidateEvidenceClass -Source $Source -Rows $Rows
    }
}
...
function Convert-CandidatesToOllamaPayload {
    ...
    foreach ($candidate in @($Candidates | Sort-Object -Property Score, Confidence, Count -Descending | Select-Object -First 5)) {
        ...
        $payload += [pscustomobject]@{
            ...
            evidence_class = $(if ($candidate.PSObject.Properties.Name -contains 'EvidenceClass') { $candidate.EvidenceClass } else { Get-PCDECandidateEvidenceClass -Source $candidate.Source -Rows $candidate.Rows })
...
function Get-TopCandidateSet {
    ...
    if ($MaxCandidates -le 0) {
        $MaxCandidates = 3
    }

    return @($Candidates | Sort-Object -Property Score, Confidence, Count -Descending | Select-Object -First $MaxCandidates)
}

function Get-BlendedEvidenceRows {
    param(
        [object[]]$Candidates = @(),
        [int]$MaxCandidates = 3,
        [int]$MaxRowsPerCandidate = 5
    )

    $topCandidates = @(Get-TopCandidateSet -Candidates $Candidates -MaxCandidates $MaxCandidates)
    ...
    foreach ($candidate in $topCandidates) {
        foreach ($row in @($candidate.Rows) | Select-Object -First $MaxRowsPerCandidate) {
            $rows.Add($row)
        }
    }
...
function Get-EvidenceSourceSummary {
    param(
        [object[]]$Candidates = @(),
        [int]$MaxCandidates = 5
    )
    ...
    $labels = @($topCandidates | ForEach-Object { $_.Label } | Select-Object -Unique)
    if ($labels.Count -eq 0) {
        $labels = @($topCandidates | ForEach-Object { $_.Source } | Select-Object -Unique)
    }

    return ($labels -join ", ")
}

function Get-EvidenceSourceTag {
    param(
        [object[]]$Candidates = @(),
        [string]$DefaultSource = "unknown"
    )

    $topCandidates = @(Get-TopCandidateSet -Candidates $Candidates -MaxCandidates 3)
    if ($topCandidates.Count -le 0) {
        return $DefaultSource
    }

    if ($topCandidates.Count -eq 1) {
        return [string]$topCandidates[0].Source
    }

    return "multi_source"
}

function Get-EvidenceConfidence {
    param(
        [object[]]$Candidates = @()
    )
    ...
    $avg = ($topCandidates | Measure-Object -Property Confidence -Average).Average
    if ($null -eq $avg) {
        return 0.0
    }
...
Source: $($candidate.Source)
Label: $($candidate.Label)
EvidenceClass: $(if ($candidate.PSObject.Properties.Name -contains 'EvidenceClass') { $candidate.EvidenceClass } else { Get-PCDECandidateEvidenceClass -Source $candidate.Source -Rows $candidate.Rows })
Score: $($candidate.Score)
Confidence: $($candidate.Confidence)
Evidence:
$preview
...
function Resolve-RecallQueryType {
    ...
    if ($normalized -match '^(how\s+do\s+you|how\s+do\s+we|how\s+to|how\s+can\s+i)\b') {
        return 'procedural'
    }
...
function Invoke-OllamaMemoryArbiter {
    ...
    $judgementJson = if ($null -ne $JudgementContext) {
        try { $JudgementContext | ConvertTo-Json -Depth 6 -Compress } catch { "{}" }
    } else { "{}" }

    $prompt = @"
You are Mira, the reasoning layer for PCDE/MiraTV.

You are operating inside the PCDE cognitive system. Retrieved system memory is preferred over general knowledge.
...
Your primary task is to answer the ORIGINAL USER QUESTION using the retrieved evidence.

CRITICAL RULES:
- Answer the user's question directly in plain language.
- Do NOT return raw rows, key-value pairs, field mappings, relation labels, or database-style outputs.
- Do NOT echo memory records as the answer.
- Do NOT define the memory tier labels unless the user explicitly asks about the memory system.
- If the question asks what something is, provide a definition first.
- Prefer higher-ranked, system-specific evidence over incidental keyword matches.
- Prefer direct definitional evidence over headers, procedure names, or domain tags.
- Treat relational evidence as authoritative when it directly answers the question.
- Treat vector evidence as support/context unless no authoritative evidence exists.
- Use contextual evidence only to support or clarify.
- If evidence is weak or contradictory, say that briefly, then answer cautiously.
- Return JSON only.
...
{
  "selected_source": "procedural|declarative|associative|working|ai_memory|long_term|synthetic_procedures|multi_source|none",
  "selected_label": "label or none",
  "grounded": true,
  "confidence": 0.0,
  "answer": "answer the user's original question directly in natural language",
...
function Ask-AI {
    ...
    if ($memoryCandidates.Count -gt 0) {
        ...
        $memorySource = Get-EvidenceSourceTag -Candidates $topCandidateSet -DefaultSource $bestCandidate.Source
...
"@
    ...
    if ($judgementDecision.NeedsRecallOverride -eq $true) {
    ...
    if ($pass1.needs_long_term_recall -eq $true) {
        ...
        if ($recallTerms.Count -eq 0) {
            $recallTerms = @($pass1.recall_terms | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
        }
        if ($recallTerms.Count -eq 0) {
    ...
    $maxCandidates = $null
    try { $maxCandidates = $reasoningRules.candidate_selection.max_candidates_sent_to_arbiter } catch { $maxCandidates = $null }
    if (-not $maxCandidates) { $maxCandidates = 8 }
