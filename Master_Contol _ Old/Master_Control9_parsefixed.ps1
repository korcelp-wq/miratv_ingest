...
"@
        & $queryScript -Sql $commandLogSql 2>$null | Out-Null
...
"@
            & $queryScript -Sql $workingMemorySql 2>$null | Out-Null
...
'@ }
                    ...
                    if ($scriptBlock) {
                        $startJobCommand = @"
Start-Job -Name '$actualService' -ScriptBlock ([ScriptBlock]::Create(@'
$scriptBlock
'@))
"@
                        $result = Invoke-PowerShellCommand -Command $startJobCommand -Description "Starting $actualService"
                        $answer = if ($result.success) { 
                            "✅ Started $actualService successfully as a continuous background job."
                        } else { 
                            "❌ Failed to start $actualService`: $($result.output)"
                        }
                    } else {
