...
"@
    ...
    if ($memoryCandidates.Count -gt 0) {
        ...
        Write-Host "   🏆 Best match selected: $($bestCandidate.Label) (score=$($bestCandidate.Score), confidence=$($bestCandidate.Confidence), rows=$($bestCandidate.Count))" -ForegroundColor Magenta
    }
    ...
    if (-not $memoryResult -and ($Question -match "procedures?" -or $Question -match "what (procedures|services) (do you have|are available)")) {
        ...
        if ($procedureCount -and $procedureCount[0].count -gt 0) {
            $count = [int]$procedureCount[0].count
...
"@
            ...
            if ($procedureList) {
                $formattedList = Format-MemoryRows -Rows $procedureList -Kind "procedures"
                $formattedAnswer = @"
...
"@ 4>$null | Out-Null
    ...
    if ($memoryResult) {
        ...
        if ($memoryCandidates -and $memoryCandidates.Count -gt 0) {
            $evaluationSummary = (($memoryCandidates | Sort-Object -Property Score -Descending | ForEach-Object {
                "- $($_.Label): rows=$($_.Count), confidence=$($_.Confidence), score=$($_.Score)"
            }) -join "`n")
        }

        $formattedAnswer = @"
$icon Best match selected from evaluated memory tiers: $memorySource
