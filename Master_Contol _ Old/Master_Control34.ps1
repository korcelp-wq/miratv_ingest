...
function Invoke-PCDELongTermRecall {
    ...
    Write-Host "   ?? Calling long-term recall stored procedure..." -ForegroundColor Cyan
    ...
    try {
        ...
        if ($rows -and $rows.Count -gt 0) {
            Write-Host "   [OK] Long-term recall returned $($rows.Count) row(s)" -ForegroundColor Green
        }
        else {
