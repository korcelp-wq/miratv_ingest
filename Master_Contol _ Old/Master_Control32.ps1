...
function Invoke-PCDELongTermRecall {
    ...
    Write-Host "   🔍 Searching ALL tables across both databases..." -ForegroundColor Cyan
    Write-Host "      Term: '$SearchTerm'" -ForegroundColor DarkGray
    Write-Host "      Type: $QueryType" -ForegroundColor DarkGray
    Write-Host "      Limit per table: $MaxRowsPerTarget" -ForegroundColor DarkGray
    
    # Escape special characters for SQL LIKE
    $escapedTerm = $SearchTerm -replace "'", "''"
    $escapedTerm = $escapedTerm -replace "\\", "\\\\"
    $escapedTerm = $escapedTerm -replace "%", "\\%"
    $escapedTerm = $escapedTerm -replace "_", "\\_"
...
"@
        ...
        try {
            ...
            if ($rows -and $rows.Count -gt 0) {
                $tablesWithData++
                Write-Host "   📁 $($target.Name): $($rows.Count) match(es)" -ForegroundColor Green
                $allRows += $rows
            }
        }
        catch {
            ...
            # Write-Host "   ⚠️ Error in $($target.Name): $($_.Exception.Message)" -ForegroundColor DarkGray
        }
    }
    
    if ($allRows.Count -gt 0) {
        Write-Host "   ✅ Found $($allRows.Count) total matches across $tablesWithData table(s)" -ForegroundColor Green
    } else {
        Write-Host "   ⚪ No matches found in any table" -ForegroundColor DarkGray
    }
...
function Test-LongTermMemoryTables {
    ...
    foreach ($target in $targets) {
        ...
        try {
            ...
            Write-Host "✅ $($target.Name) - Accessible" -ForegroundColor Green
        }
        catch {
            $missingTables += $target.Name
            Write-Host "❌ $($target.Name) - Not accessible: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
