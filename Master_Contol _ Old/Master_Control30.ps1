...
function Format-MemoryRows {
    ...
    $lines = foreach ($row in @($Rows) | Select-Object -First 5) {
        ...
        elseif ($props -contains 'preview') {
            $src = if ($props -contains 'source_name' -and $row.source_name) { $row.source_name }                    elseif ($props -contains 'source_table' -and $row.source_table) { $row.source_table }                    else { 'long_term' }
            "- [$src] $($row.preview)"
        }
...
function Show-MemoryRowsPreview {
    ...
    $previewLines = @(Format-MemoryRows -Rows @($Rows | Select-Object -First $MaxLines) -Kind $Kind -split "`n")
...
function Get-PCDELongTermSearchTargets {
    return @(
        ...
        [pscustomobject]@{ Name = "Lake Knowledge - extracted_docs"; Database = "xpdgxfsp_lake_knowledge"; Table = "extracted_docs"; IdColumn = "id"; PreviewExpression = "LEFT(CONCAT_WS(' | ', COALESCE(title, ''), COALESCE(content, ''), COALESCE(source_ref, '')), 220)"; PreviewAlias = "preview"; WhereClause = "(content LIKE '%{0}%' ESCAPE '\\' OR title LIKE '%{0}%' ESCAPE '\\' OR source_ref LIKE '%{0}%' ESCAPE '\\')" },
        [pscustomobject]@{ Name = "Lake Knowledge - doc_sections"; Database = "xpdgxfsp_lake_knowledge"; Table = "doc_sections"; IdColumn = "id"; PreviewExpression = "LEFT(CONCAT_WS(' | ', COALESCE(title, ''), COALESCE(content, '')), 220)"; PreviewAlias = "preview"; WhereClause = "(content LIKE '%{0}%' ESCAPE '\\' OR title LIKE '%{0}%' ESCAPE '\\')" },
        ...
        [pscustomobject]@{ Name = "Lake Vector - doc_sections"; Database = "xpdgxfsp_lake_vector"; Table = "doc_sections"; IdColumn = "id"; PreviewExpression = "LEFT(CONCAT_WS(' | ', COALESCE(title, ''), COALESCE(content, '')), 220)"; PreviewAlias = "preview"; WhereClause = "(content LIKE '%{0}%' ESCAPE '\\' OR title LIKE '%{0}%' ESCAPE '\\')" },
...
function Invoke-PCDELongTermRecall {
    ...
    $escapedTerm = $SearchTerm.Replace("'", "''").Replace("%", "\\%").Replace("_", "\\_")
...
SELECT
    ...
    '$($target.Name)' AS source_name,
    '$($target.Database)' AS source_db,
    '$($target.Table)' AS source_table,
    $($target.PreviewExpression) AS preview
FROM $($target.Table)
WHERE $where
...
"@
        ...
        try {
            $rows = @(Invoke-SqlQueryObjects -Sql $sql -DatabaseName $target.Database)
            foreach ($row in $rows) {
                if (-not ($row.PSObject.Properties.Name -contains 'preview')) {
                    $row | Add-Member -NotePropertyName preview -NotePropertyValue '' -Force
                }
                $allRows += $row
            }
        }
...
function Ask-AI {
    ...
    try {
        ...
        Write-Host "   ⚠️ Could not load reasoning rules YAML. Using fallback routing." -ForegroundColor Yellow
    ...
    foreach ($memoryType in @($routingPlan.PreferredOrder)) {
        ...
        try {
            ...
            if ($rows -and @($rows).Count -gt 0) {
                Write-Host ("   ✅ Found in {0} ({1} rows)" -f $label, @($rows).Count) -ForegroundColor Green
                ...
                Write-Host ("   ⚪ No results in {0}" -f $label) -ForegroundColor DarkGray
            }
        }
        catch {
    ...
    if ($memoryCandidates.Count -gt 0) {
        ...
        if ($reasoningRules.debug.show_selected_evidence_set -ne $false) {
            Write-Host ("   🏆 Selected evidence set: {0}" -f $bestCandidate.Label) -ForegroundColor Magenta
        }
    }
...
"@
    ...
    Write-Host ("   🧠 Pass1 -> sufficient={0} recall={1}" -f $pass1.sufficient, $pass1.needs_long_term_recall) -ForegroundColor Cyan
    ...
    if ($pass1.needs_long_term_recall -eq $true) {
        ...
        Write-Host ("   🔁 Long-term recall: {0}" -f $longTermSearch) -ForegroundColor Yellow

        $longTermRows = @(Invoke-PCDELongTermRecall -SearchTerm $longTermSearch -MaxRowsPerTarget 2)
        if ($longTermRows.Count -gt 0) {
            Write-Host ("   ✅ Long-term recall returned {0} rows" -f $longTermRows.Count) -ForegroundColor Green
            ...
            if ($reasoningRules.debug.show_selected_evidence_set -ne $false) {
                Write-Host ("   🏆 Reconciled evidence set: {0}" -f $bestCandidate.Label) -ForegroundColor Magenta
            }
        }
        else {
            Write-Host "   ⚪ Long-term recall returned no additional rows" -ForegroundColor DarkGray
        }
    }
